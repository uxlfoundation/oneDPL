...
class token_policy : public policy_base<token_policy<ResourceType, ResourceAdapter, Backend>, ResourceType, Backend>
{
  ...
  protected:
    ...
    struct token_t
    {
        ...
        ~token_t() { token_availability_.fetch_sub(1); }
    };
    ...
    class token_selection_handle_t
    {
      ...
      public:
        token_selection_handle_t(const Policy& p, std::shared_ptr<resource_t> r, std::shared_ptr<token_t> t)
            : policy_(p), resource_(std::move(r)), token_(std::move(t))
        {
        } ///
        auto
        unwrap()
  ...
  public:
    ...
    token_policy(const std::vector<resource_type>& u, ResourceAdapter adapter = {}, const int& c = 1) : capacity(c)
    {
