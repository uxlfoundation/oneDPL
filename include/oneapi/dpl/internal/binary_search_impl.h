...
struct __lower_bound_impl_fn_fo
{
    InputIterator start;
    InputIterator end;
...
struct __upper_bound_impl_fn_fo
{
    ...
    auto operator()(const _ValueType& val) const -> declval(std::upper_bound(start, end, val, comp) - start)
    {
...
struct __binary_search_impl_fn_fo
{
    ...
    auto
    operator()(const _ValueType& val) const -> declval(std::binary_search(start, end, val, comp))
    {
