...
template <int sub_group_size, int iters_per_item,
          ...
          typename NdItem, typename SlmAcc, typename ArrayOrder,
          typename BinaryOperation>
auto
work_group_scan(ArrayOrder, const NdItem& item, SlmAcc local_acc,
                InputType input[iters_per_item], /*OutputType output[iters_per_item],*/BinaryOperation binary_op,
                uint32_t items_in_scan)
{
    ...
    if constexpr (std::is_same_v<ArrayOrder, item_array_order::sub_group_stride>)
    {
        ...
        auto sub_group_carry = sub_group_scan<sub_group_size, iters_per_item>(ArrayOrder{}, sub_group, input, /*output,*/ binary_op);
        const std::uint8_t sub_group_group_id = sub_group.get_group_linear_id();
        const std::uint8_t active_sub_groups = oneapi::dpl::__internal::__dpl_ceiling_div(items_in_scan, sub_group_size * iters_per_item);
        ...
        if (sub_group_group_id == 0)
        {
            ...
            if (active_sub_groups == num_sub_groups) {
                ...
                auto val = local_acc[idx];
                __sub_group_scan_partial<sub_group_size, true, true>(sub_group, val, binary_op, wg_carry, active_sub_groups % sub_group_size);
                local_acc[idx] = val;
            }
        }
        sycl::group_barrier(item.get_group());
...
work_group_scan(ArrayOrder, const NdItem& item, SlmAcc local_acc,
                InputType input[iters_per_item], /*OutputType output[iters_per_item],*/BinaryOperation binary_op)
{
    return work_group_scan<sub_group_size, iters_per_item>(ArrayOrder{}, item, local_acc, input, binary_op, item.get_local_range()[0] * iters_per_item);
}
