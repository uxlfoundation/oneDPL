...
#ifndef _ONEDPL_KT_SYCL_RADIX_SORT_DISPATCHERS_H
#define _ONEDPL_KT_SYCL_RADIX_SORT_DISPATCHERS_H
...
#include "sycl_radix_sort_submitters.h"

namespace oneapi::dpl::experimental::kt::gpu::__sycl::__impl
{

//-----------------------------------------------------------------------------
// Memory holder: Manages temporary buffers for onesweep
//-----------------------------------------------------------------------------
...
class __onesweep_memory_holder
{
    ...
    _HistT* __m_global_hist_ptr = nullptr;  // Global histograms (scanned)
    _HistT* __m_group_hist_ptr = nullptr;   // Group histograms (for lookback)
    _KeyT* __m_keys_ptr = nullptr;          // Temporary key buffer
    _ValT* __m_vals_ptr = nullptr;          // Temporary value buffer (if key-value)
    ...
    {
        // Extra bytes for alignment padding
    ...
    {
        // Non-typed allocation is guaranteed to be aligned for any fundamental type
...
//-----------------------------------------------------------------------------
// Onesweep implementation kernel launcher
//-----------------------------------------------------------------------------
template <typename _KernelName, bool __is_ascending, std::uint8_t __radix_bits,
          std::uint16_t __data_per_work_item, std::uint16_t __work_group_size,
          typename _RngPack1, typename _RngPack2, typename _RngPack3, typename _MemHolder>
sycl::event
__onesweep_impl(sycl::queue __q, _RngPack1&& __input_pack, _RngPack2&& __virt_pack1,
                _RngPack3&& __virt_pack2, const _MemHolder& __mem_holder, std::size_t __n)
{
    ...
    using _HistogramKernel = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
        __sycl_radix_sort_onesweep_histogram<_KernelName>>;
    using _ScanKernel = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
        __sycl_radix_sort_onesweep_scan<_KernelName>>;
    using _SweepInitialKernel = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
        std::conditional_t<__has_values,
                          __sycl_radix_sort_onesweep_by_key<std::decay_t<_RngPack1>, std::decay_t<_RngPack2>, _KernelName>,
                          __sycl_radix_sort_onesweep<std::decay_t<_RngPack1>, std::decay_t<_RngPack2>, _KernelName>>>;
    using _SweepEvenKernel = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
        std::conditional_t<__has_values,
                          __sycl_radix_sort_onesweep_by_key<std::decay_t<_RngPack3>, std::decay_t<_RngPack2>, _KernelName>,
                          __sycl_radix_sort_onesweep<std::decay_t<_RngPack3>, std::decay_t<_RngPack2>, _KernelName>>>;
    using _SweepOddKernel = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
        std::conditional_t<__has_values,
                          __sycl_radix_sort_onesweep_by_key<std::decay_t<_RngPack2>, std::decay_t<_RngPack3>, _KernelName>,
                          __sycl_radix_sort_onesweep<std::decay_t<_RngPack2>, std::decay_t<_RngPack3>, _KernelName>>>;
    ...
    // Phase 1: Compute global histograms (for ALL stages at once)
    // Zero-initialize both global and group histograms (stored sequentially)
    ...
    // TODO: Make histogram work-group config tunable
    constexpr std::uint32_t __hist_work_group_count = 64;
    constexpr std::uint32_t __hist_work_group_size = 64;
    __event_chain = __radix_sort_histogram_submitter<__is_ascending, __radix_bits,
    ...
    // Phase 2: Scan global histograms to produce offsets
    __event_chain = __radix_sort_onesweep_scan_submitter<__stage_count, __bin_count, _ScanKernel>()(
        __q, __mem_holder.__global_hist_ptr(), __event_chain);

    // Phase 3: Sweep stages (reorder data stage by stage)
    // Stage 0: input -> virt_pack1
    __event_chain = __radix_sort_onesweep_submitter<__is_ascending, __radix_bits, __data_per_work_item,
                                                    __work_group_size, _SweepInitialKernel>()(
    ...
    // Subsequent stages: ping-pong between buffers
    for (std::uint32_t __stage = 1; __stage < __stage_count; __stage++)
    {
        ...
        if (__stage % 2 != 0)
        {
            // Odd stage: virt_pack1 -> virt_pack2
            __event_chain = __radix_sort_onesweep_submitter<__is_ascending, __radix_bits, __data_per_work_item,
                                                            __work_group_size, _SweepOddKernel>()(
                __q, __virt_pack1, __virt_pack2, __p_global_hist, __p_group_hists, __sweep_work_group_count, __n,
                __stage, __event_chain);
        }
        else
        {
            // Even stage: virt_pack2 -> virt_pack1
            __event_chain = __radix_sort_onesweep_submitter<__is_ascending, __radix_bits, __data_per_work_item,
                                                            __work_group_size, _SweepEvenKernel>()(
...
//-----------------------------------------------------------------------------
// Onesweep dispatcher: Sets up memory and launches kernels
//-----------------------------------------------------------------------------
template <typename _KernelName, bool __is_ascending, std::uint8_t __radix_bits,
          std::uint16_t __data_per_work_item, std::uint16_t __work_group_size,
          bool __in_place, typename _RngPack1, typename _RngPack2>
sycl::event
__onesweep(sycl::queue __q, _RngPack1&& __pack, _RngPack2&& __pack_out, std::size_t __n)
{
    ...
    using _CopybackKernel = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
        std::conditional_t<__has_values, __sycl_radix_sort_onesweep_copyback_by_key<_KernelName>,
                          __sycl_radix_sort_onesweep_copyback<_KernelName>>>;
    ...
    // Create temporary pack for ping-pong
    auto __get_tmp_pack = [&]() {
    ...
    // Determine which buffers to use based on in-place and stage count
    ...
    // Phase 4: Copyback if needed (in-place with odd stages)
    if constexpr (__in_place && (__stage_count % 2 != 0))
    {
        __event_chain =
            __radix_sort_copyback_submitter<_CopybackKernel>()(__q, __tmp_pack, __pack, __n, __event_chain);
    }

    // Deallocate temporary memory asynchronously
    return __mem_holder.__async_deallocate(__event_chain);
}

//-----------------------------------------------------------------------------
// Main entry point: Radix sort dispatcher
//-----------------------------------------------------------------------------
template <bool __is_ascending, std::uint8_t __radix_bits, bool __in_place,
          typename _RngPack1, typename _RngPack2, typename _KernelParam>
sycl::event
__radix_sort(sycl::queue __q, _RngPack1&& __pack_in, _RngPack2&& __pack_out, _KernelParam)
{
    ...
    // Validate parameters
    __check_onesweep_params<__radix_bits, __data_per_workitem, __workgroup_size>();

    // For now, always use onesweep (no one-workgroup optimization yet)
    // TODO: Add one-workgroup optimization for small inputs
...
#endif // _ONEDPL_KT_SYCL_RADIX_SORT_DISPATCHERS_H
