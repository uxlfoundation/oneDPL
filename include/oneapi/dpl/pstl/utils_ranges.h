...
template <typename _R>
struct __has_subscription_op<_R,
                             std::void_t<decltype(std::declval<const std::remove_reference_t<_R>&>().operator[](0))>>
    : std::true_type
{
};
...
struct __subscription_impl_view_simple : std::ranges::view_interface<__subscription_impl_view_simple<_View>>
{
    ...
    // mutable to support views which are not const-iterable (e.g. std::ranges::reverse_view over a non-common range)
