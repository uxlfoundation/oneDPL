...
struct __tuple_util
{
    ...
    {
        ::std::get<_Np - 1>(__it) = ::std::get<_Np - 1>(__it) + __forward; // KSATODO fix get
    ...
    {
        ++::std::get<_Np - 1>(__it); // KSATODO fix get
...
struct __make_references
{
    ...
    {
        return _TupleReturnType(*::std::get<_Ip>(__t)...); // KSATODO fix get
    }
};
...
class zip_forward_iterator
{
  ...
  public:
    ...
    {
        return ::std::get<0>(__my_it_) == ::std::get<0>(__it.__my_it_); // KSATODO fix get
    }
    bool
    operator!=(const zip_forward_iterator& __it) const
    {
