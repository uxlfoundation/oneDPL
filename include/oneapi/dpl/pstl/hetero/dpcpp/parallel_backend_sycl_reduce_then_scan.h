...
// With optimization enabled, reduce-then-scan requires a sub-group size of 32. Without optimization, we use a sub-group
// size of 16 on Intel GPUs (detected as SPIRV targets) and 32 everywhere else. This works around a hardware bug on
// certain Intel graphics with older drivers.
//
// When querying from the host compiler, return a value of 32 always to ensure a generic non-Intel device supports this
// sub-group size. For the case of Intel GPUs, no additional check for sub-group sizes of 16 is performed on the host as
// we know this is supported.
constexpr inline std::uint8_t
__get_reduce_then_scan_sg_sz()
{
    return
#if !defined(__SYCL_DEVICE_ONLY__) || _ONEDPL_DETECT_COMPILER_OPTIMIZATIONS_ENABLED || !_ONEDPL_DETECT_SPIRV_COMPILATION
        32;
#else
        16;
#endif
    ...
    _ScanInputTransform, _WriteOp, _InitType, __internal::__optional_kernel_name<_KernelName...>>
{
    ...
    operator()(_ExecutionPolicy&& __exec, const sycl::nd_range<1> __nd_range, _InRng&& __in_rng, _OutRng&& __out_rng,
               _TmpStorageAcc& __scratch_container, const sycl::event& __prior_event, const std::size_t __inputs_remaining,
               const std::size_t __block_num) const
    {
            ...
            __cgh.parallel_for<_KernelName...>(
                ...
                else
                {
                    if (__sub_group_id > 0)
                    {
                        _InitValueType __value =
                            __sub_group_partials[std::min(__sub_group_id - 1, __active_subgroups - 1)];
                        __sub_group_carry.__setup(__reduce_op(__get_block_carry_in(__block_num, __tmp_ptr, __num_sub_groups_global), __value));
                    }
                    else if (__group_id > 0)
                    {
                        __sub_group_carry.__setup(__reduce_op(__get_block_carry_in(__block_num, __tmp_ptr, __num_sub_groups_global),
                                                              __sub_group_partials[__active_subgroups]));
                    }
                    else
                    {
                        __sub_group_carry.__setup(__get_block_carry_in(__block_num, __tmp_ptr, __num_sub_groups_global));
                    }
                }
                    ...
                    (__active_subgroups == __sub_group_id + 1))
                {
                    ...
                    else
                    {
                        ...
                        __set_block_carry_out(__block_num, __tmp_ptr, __sub_group_carry.__v, __num_sub_groups_global);
                    }
                }
                __sub_group_carry.__destroy();
            });
