...
// Sub-group size used inside the kernel. When compiling for SPIR-V the reqd_sub_group_size attribute is honored, so
// the size is a compile-time constant and every sub-group in the work-group is that size (loops stay unrolled, the
// broadcast lane id is known). Otherwise the attribute may be ignored and sub-group sizes are implementation-defined
...
// SPIR-V path it is a compile-time constant (see _ONEDPL_RTS_SUB_GROUP_SIZE), and obtaining it via the macro at each
// use keeps loop bounds and broadcast ids constant-folded; caching it in a member would turn those into runtime loads.
            ...
            __cgh.parallel_for<_KernelName...>(
                ...
                if (__sub_group_id < __active_subgroups)
                {
                    ...
                    __scan_through_elements_helper</*_Bounded*/ false, __is_inclusive, /*__init_present=*/false,
                        ...
                        __wi, __gen_reduce_input, oneapi::dpl::identity{}, __reduce_op, nullptr, __sub_group_carry,
                                           ...
                                           _TransformResult __transform_result, sycl::event __prior_event)
{
    ...
    for (std::size_t __b = 0; __b < __num_blocks; ++__b)
    {
        ...
        __prior_event = __reduce_submitter(__q, __kernel_nd_range, __in_rng, __result_and_scratch, __prior_event,
                                           __inputs_per_item, __b, __oob_pos_storage);
        ...
        __prior_event = __scan_submitter(__q, __kernel_nd_range, __in_rng, __out_rng, __result_and_scratch,
                                         __prior_event, __inputs_per_item, __b, __oob_pos_storage);
