...
struct __combine_set_operation_offsets
{
    ...
    {
        return {std::max(__arg1.get<0>(), __arg2.get<0>()), // Max offset in the first input range
                std::max(__arg1.get<1>(), __arg2.get<1>()), // Max offset in the second input range
                __arg1.get<2>() + __arg2.get<2>()};         // Sum of offsets in the output range (amount of filled elements)
    }
};
