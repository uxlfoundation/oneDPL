...
struct is_temp_buff<_Iter, std::enable_if_t<!is_sycl_iterator_v<_Iter> && !std::is_pointer_v<_Iter> &&
                                            !oneapi::dpl::__internal::is_indirectly_device_accessible_iterator_v<_Iter>>>
    : std::true_type
{
};
...
struct __get_sycl_range
{
  ...
  private:
    ...
    template <sycl::access::mode _LocalAccMode, typename _Iter>
    std::enable_if_t<oneapi::dpl::__internal::is_indirectly_device_accessible_iterator_v<_Iter>,
