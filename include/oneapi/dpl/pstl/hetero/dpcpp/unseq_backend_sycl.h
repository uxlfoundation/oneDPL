...
struct walk_n_vectors_or_scalars
{
  private:
    ...
    struct load_all_but_last
    {
        ...
        void
        operator()(std::integral_constant<std::size_t, I>, Rng&& rng)
        {
    ...
    template <typename _IsFull, typename _Params, std::size_t... _Is, std::size_t... _Js, typename... _Ranges,
              std::enable_if_t<_Params::__b_vectorize, int> = 0>
    void
    __vector_impl(_IsFull __is_full, const std::size_t __idx, _Params, std::integer_sequence<std::size_t, _Is...>,
                  std::integer_sequence<std::size_t, _Js...>, _Ranges&&... __rngs) const
    {
        ...
        load_all_but_last<_IsFull, __scalar_load_op, _LastDroppedTuple, _Params::__vector_size, sizeof...(_Ranges)>
            __loader{__idx, __ld, __vec_load, __load_op};
        (__loader(std::integral_constant<std::size_t, _Is>{}, __rngs), ...);
        auto load_and_store_tup =
            std::tuple_cat(__ld, std::make_tuple(std::get<sizeof...(_Ranges) - 1>(std::tuple<_Ranges...>(__rngs...))));
  ...
  public:
    ...
    operator()(_IsFull __is_full, const std::size_t __idx, _Params, _Ranges&&... __rngs) const
    {
        ...
        else
        {
            ...
            __vector_impl(__is_full, __idx, _Params{}, __indices, __indices_dropped, std::forward<_Ranges>(__rngs)...);
        }
    }
