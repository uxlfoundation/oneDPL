...
__parallel_transform_scan(oneapi::dpl::__internal::__device_backend_tag, _ExecutionPolicy&& __exec,
                          _Range1&& __in_rng, _Range2&& __out_rng, 
                                              ...
                                              _BinaryPredicate __binary_pred, _BinaryOperator __binary_op)
{
    ...
    return __parallel_transform_reduce_then_scan<sizeof(oneapi::dpl::__internal::tuple<std::size_t, _ValueType>),
                                                 _CustomName>(
        __q, __n,
        std::move(__zip_view_keys_vals_in), std::move(__zip_view_keys_vals_out),
        _GenReduceInput{__binary_pred},
        _ReduceOp{__binary_op},
        _GenScanInput{__binary_pred, __n},
        _ScanInputTransform{},
...
oneapi::dpl::__ranges::__internal::__rng_set_operations_return_t<_Range1, _Range2, _Range3>
                                            ...
                                            _Compare __comp, _Proj1 __proj1, _Proj2 __proj2)
{
    ...
    return __parallel_transform_reduce_then_scan<sizeof(oneapi::dpl::__internal::__value_t<_Range1>), _CustomName>(
        ...
        _ReduceOp{},
        _GenScanInput{_GenMaskScan{_MaskPredicate{}, _MaskRangeTransform{}},
        _ScanRangeTransform{}},
        _ScanInputTransform{},
        _WriteOp{},
        oneapi::dpl::unseq_backend::__no_init_value<_Size>{},
...
#else
    return oneapi::dpl::__ranges::__internal::__rng_set_operations_return_t<_Range1, _Range2, _Range3>{};
#endif
}
                      ...
                      _Compare __comp, _Proj1 __proj1, _Proj2 __proj2)
{
    ...
    auto [__first1, __last1, __n1] = oneapi::dpl::__ranges::__get_range_bounds(__rng1);
    auto [__first2, __last2, __n2] = oneapi::dpl::__ranges::__get_range_bounds(__rng2);
    auto [__result1, __result2, __n_out] = oneapi::dpl::__ranges::__get_range_bounds(__result);

    // temporary buffer to store intermediate result
    const auto __diff_buffer_size = std::min(__n2, __n_out);
    oneapi::dpl::__par_backend_hetero::__buffer<_ValueType> __diff_buffer(__diff_buffer_size);
    ...
    else
    {
        ...
        // __merge_result.first - merged from the first range
        // __merge_result.second - merged from the second range
        auto __merge_result =
                      ...
                      _Compare __comp, _Proj1 __proj1, _Proj2 __proj2)
{
    ...
    {
        // TODO check returns type of __parallel_set_reduce_then_scan_set_a_write
        // -> __future<sycl::event, 
    ...
    {
        // TODO check returns type of __parallel_set_scan
        // -> __future<sycl::event,
        //             __result_and_scratch_storage<
        //                 oneapi::dpl::__internal::__difference_t<_Range1>
        //             >
