...
struct __red_by_seg_op
{
    ...
    operator()(const _Tup1& __lhs_tup, const _Tup2& __rhs_tup) const
    {
        ...
        return oneapi::dpl::__internal::make_tuple(get<0>(__lhs_tup) + get<0>(__rhs_tup), // KSATODO fix get
                                                   _OpReturnType{get<1>(__rhs_tup)}); // KSATODO fix get
    }
...
struct __extract_range_from_zip
{
    ...
    {
        return std::get<_EleId>(__in_rng.tuple()); // KSATODO fix get?
    }
};
...
struct __parallel_reduce_by_segment_fallback_fn1
{
    ...
    operator()(const T& __a) const
    {
        ...
        return index % __wgroup_size == 0                             // segment size
               || !__binary_pred(std::get<1>(__a), std::get<2>(__a)); // key comparison // KSATODO fix get
    }
};
...
struct __parallel_reduce_by_segment_fallback_fn2
{
    ...
    operator()(const T& __a) const
    {
        ...
        if (std::get<0>(__a) == 0)
            return true;
        return !__binary_pred(std::get<1>(__a), std::get<2>(__a)); // keys comparison // KSATODO fix get
    }
};
} // namespace __internal
