...
__print_device_debug_info(const sycl::queue& __q, size_t __wg_size = 0, size_t __max_cu = 0)
{
    ...
    ::std::cout << " > max compute units:   "
                << (__max_cu ? __max_cu : oneapi::dpl::__internal::__max_compute_units(__q)) << ::std::endl;
...
{
    __print_device_debug_info(__policy.queue(), __wg_size, __max_cu);
}
#else
template <typename _Policy>
inline void
...
struct __result_and_scratch_storage : __result_and_scratch_storage_base
{
  ...
  public:
    __result_and_scratch_storage(sycl::queue __q, std::size_t __result_n, std::size_t __scratch_n)
