...
struct __combined_storage : public __device_storage<_T>
{
    ...
    // __scratch_n may be zero: in that case no scratch memory is allocated at all.
    ...
    __move_state() &&
    {
        return {std::move(__result_buf),
                std::move(this->__usm_buf),
                std::move(this->__sycl_buf),
                __result_sz,
                __sz,
                __kind};
    }
...
{
    __copyable_storage_state<_T> __data = std::move(__cst).__move_state();
    assert(__data.__result_sz <= 1);
...
__create_future(std::tuple<sycl::event, _Args...> __res, _ExtraArgs&&... __extra)
{
    ...
    // Convert __combined_storage items into __result_and_scratch_storage
    return std::apply(
