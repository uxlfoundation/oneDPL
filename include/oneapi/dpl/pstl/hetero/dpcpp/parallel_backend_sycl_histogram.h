                                   ...
                                   _Range2&& __bins, const _BinHashMgr& __binhash_manager)
{
    ...
    {
        return std::tuple(__histogram_general_registers_local_reduction<_CustomName, __iters_per_work_item,
    ...
    // if bins fit into SLM, use local atomics
             ...
             __local_mem_size)
    {
        return std::tuple(__histogram_general_local_atomics<_CustomName, __iters_per_work_item>(
            __q, __init_event, __work_group_size, std::forward<_Range1>(__input), std::forward<_Range2>(__bins),
            __binhash_manager));
    }
    else // otherwise, use global atomics (private copies per workgroup)
    {
        ...
        return std::tuple(__histogram_general_private_global_atomics<_CustomName>(
