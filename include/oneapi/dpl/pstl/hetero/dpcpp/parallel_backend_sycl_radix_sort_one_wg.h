...
struct __subgroup_radix_sort
{
    ...
    operator()(sycl::queue& __q, _RangeIn&& __src, _Proj __proj, std::size_t __max_wg_size)
    {
        ...
        __wg_size = std::min(__wg_size, static_cast<std::uint16_t>(__max_wg_size));
                ...
                __cgh.parallel_for<_Name...>(
                    __range, ([=](sycl::nd_item<1> __it) [[_ONEDPL_SYCL_REQD_SUB_GROUP_SIZE_IF_SUPPORTED(16)]] {
                        ...
                        while (true)
                        {
                            ...
                            {
                                // Cache bin indices (1 byte each) instead of full pointers (8 bytes each)
                                // to reduce register pressure while avoiding recomputation from values
                                uint16_t __bins[__block_size];
