...
struct __parallel_merge_submitter<_OutSizeLimit, _IdType, __internal::__optional_kernel_name<_Name...>>
{
    ...
    auto
    operator()(oneapi::dpl::__par_backend_hetero::__sycl_queue_ref __q_ref, _Range1&& __rng1, _Range2&& __rng2, _Range3&& __rng3, _Compare __comp) const
    {
        ...
        const _IdType __chunk = __q_ref.queue().get_device().is_cpu() ? 128 : 4;
        ...
        if constexpr (_OutSizeLimit{})
            __p_res_storage = new __result_and_scratch_storage_t(__q_ref.queue(), 1, 0);
        else
            assert(__rng3.size() >= __n1 + __n2);

        auto __event = __q_ref.queue().submit([&__rng1, &__rng2, &__rng3, __p_res_storage, __comp, __chunk, __steps, __n, __n1,
                                        ...
                                        __internal::__optional_kernel_name<_MergeKernelName...>>
{
  private:
    ...
    sycl::event
    eval_split_points_for_groups(oneapi::dpl::__par_backend_hetero::__sycl_queue_ref __q_ref, _Range1&& __rng1, _Range2&& __rng2, _IdType __n,
                                 ...
                                 _Storage& __base_diagonals_sp_global_storage) const
    {
        ...
        return __q_ref.queue().submit([&__rng1, &__rng2, __comp, __nd_range_params, __base_diagonals_sp_global_storage, __n1, __n2,
    ...
    template <typename _Range1, typename _Range2, typename _Range3, typename _Compare, typename _Storage>
    sycl::event
    run_parallel_merge(const sycl::event& __event, oneapi::dpl::__par_backend_hetero::__sycl_queue_ref __q_ref, _Range1&& __rng1, _Range2&& __rng2,
                       ...
                       const _Storage& __base_diagonals_sp_global_storage) const
    {
        ...
        return __q_ref.queue().submit([&__event, &__rng1, &__rng2, &__rng3, __n, __comp, __nd_range_params,
  ...
  public:
    ...
    operator()(oneapi::dpl::__par_backend_hetero::__sycl_queue_ref __q_ref, _Range1&& __rng1, _Range2&& __rng2, _Range3&& __rng3, _Compare __comp) const
    {
        ...
        // Calculate nd-range parameters
        const nd_range_params __nd_range_params = eval_nd_range_params(__q_ref.queue(), __rng1, __rng2, __n);
        ...
        auto __p_base_diagonals_sp_global_storage = new __result_and_scratch_storage<__val_t>(
            __q_ref.queue(), _OutSizeLimit{} ? 1 : 0, __nd_range_params.base_diag_count + 1);
        ...
        sycl::event __event = eval_split_points_for_groups(__q_ref.queue(), __rng1, __rng2, __n, __comp, __nd_range_params,
                                                           *__p_base_diagonals_sp_global_storage);

        // Merge data using split points on each diagonal
        __event = run_parallel_merge(__event, __q_ref.queue(), __rng1, __rng2, __rng3, __comp, __nd_range_params,
                                     *__p_base_diagonals_sp_global_storage);
    ...
    else
    {
        if (__n <= std::numeric_limits<std::uint32_t>::max())
        {
            ...
            return __parallel_merge_submitter_large<_OutSizeLimit, _WiIndex, _CustomName, _DiagonalsKernelName,
                                                    _MergeKernelName>()(__exec.queue(), std::forward<_Range1>(__rng1),
