...
template <typename _WaitMode = __par_backend_hetero::__deferrable_mode,
          ...
          bool _IsOutNoInitRequested = true, typename _BackendTag, typename _ExecutionPolicy,
                       ...
                       _ForwardIterator1 __last1, _ForwardIterator2 __first2, _Function __f)
{
    ...
    auto __keep2 = oneapi::dpl::__ranges::__get_sycl_range<__output_acc_mode, _IsOutNoInitRequested>();
               ...
               _ForwardIterator1 __last1, _ForwardIterator2 __first2)
{
    ...
    auto __future = oneapi::dpl::__par_backend_hetero::__parallel_for(
        _BackendTag{}, std::forward<_ExecutionPolicy>(__exec),
        unseq_backend::__brick_swap<_Function>{_Function{}, static_cast<std::size_t>(__n)}, __n, __buf1.all_view(),
        __buf2.all_view());
...
__pattern_walk3(__hetero_tag<_BackendTag>, _ExecutionPolicy&& __exec, _ForwardIterator1 __first1,
                _ForwardIterator1 __last1, _ForwardIterator2 __first2, _ForwardIterator3 __first3, _Function __f)
{
    return __pattern_hetero_walk3<__par_backend_hetero::access_mode::write, true>(
        __hetero_tag<_BackendTag>{}, std::forward<_ExecutionPolicy>(__exec), __first1, __last1, __first2, __first3,
        __f);
}
                ...
                _Iterator2 __first2, _Iterator2 __last2, _Iterator3 __d_first, _Compare __comp)
{
    ...
    else
    {
        ...
        auto __keep3 = oneapi::dpl::__ranges::__get_sycl_range<__par_backend_hetero::access_mode::write,
                                                               /*_IsNoInitRequested=*/true>();
                            ...
                            _InIterator __last, _OutIterator __out_first, _OutIterator __out_last, _Compare __comp)
{
    ...
    // TODO: we can avoid a separate __pattern_hetero_walk2 for initial copy: it can be done during sort itself
    ...
    if (__in_size <= __out_size)
    {
        ...
        // runtime makes a dependency graph. In that case the call of __pattern_hetero_walk2 could be changed to
...
// Stage a shift through a temporary when the in-place walk - '__n' chains of 'size_res / __n' ordered
// copies - is too narrow to fill the device and deep enough to pay for two full-width passes.
...
__should_stage_shift(_BackendTag, _ExecutionPolicy&& __exec, _DiffType __n, _DiffType __size_res)
{
    ...
    //'4096' is the depth implied by the width bound below at its most permissive '__n'; the two move
    //together. Divisions, not products: '4096 * __n' overflows a 32-bit difference type.
    ...
    //Sized in the buffer's own element type: __buffer widens a std::tuple, so '_Tp' would understate it.
    using _TempTp =
                     ...
                     oneapi::dpl::__internal::__difference_t<_Range> __n)
{
    ...
    else if (__should_stage_shift<oneapi::dpl::__internal::__value_t<_Range>>(_BackendTag{}, __exec, __n, __size_res))
    {
        using _Tp = oneapi::dpl::__internal::__value_t<_Range>;

        auto __temp_buf = oneapi::dpl::__par_backend_hetero::__buffer<_Tp>(__size_res);
        //__buffer widens a std::tuple, so the views take the buffer's element type rather than '_Tp'.
        ...
        //_NoInit: this kernel overwrites the whole temporary.
        auto __temp_rng_w =
            oneapi::dpl::__ranges::all_view<_TempTp, __par_backend_hetero::access_mode::write, /*_NoInit*/ true>(
                __temp_buf.get_buffer());
        //Named policies: case 1 submits the same brick and count, so both kernels here need their own.
        oneapi::dpl::__par_backend_hetero::__parallel_for(
        ...
        //No wait between the kernels; the runtime orders them through the temporary.
        //read_write, not read: 'const _Tp&' makes the brick's std::move copy-assign, breaking move-only types.
