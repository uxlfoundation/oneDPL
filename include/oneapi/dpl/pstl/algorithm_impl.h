...
template <class _Tag, class _ExecutionPolicy, class _ForwardIterator, class _Size, class _Function>
_ForwardIterator
__pattern_for_each_n(_Tag, _ExecutionPolicy&& __exec, _ForwardIterator __first, _Size __n, _Function __f)
{
    return __pattern_walk1_n(_Tag{}, std::forward<_ExecutionPolicy>(__exec), __first, __n, __f);
}
...
_OutputIterator
__pattern_rotate_copy(_Tag __tag, _ExecutionPolicy&&, _ForwardIterator __first, _ForwardIterator __middle,
                      _ForwardIterator __last, _OutputIterator __result, std::size_t __n_out) noexcept
{
    static_assert(__is_serial_tag_v<_Tag> || __is_parallel_forward_tag_v<_Tag>);
                    ...
                    _RandomAccessIterator __last, _UnaryPredicate __pred)
{
    ...
    // and a "mirror" half [__last - __n / 2, __last).
    // Parallel reduction runs over the real half. Each leaf pairs a real chunk with its mirror chunk and
    // partitions the pair by swapping the misplaced elements. Once one side is exhausted, it
    // partitions the remaining tail on one side. This yields at most one leftover per leaf:
    // either a false-leftover (elements for which the predicate is FALSE) on the real side or
    // a true-leftover (elements for which the predicate is TRUE) on the mirror side, never both.
    //
    // Merge combines adjacent leaf ranges by moving leftovers toward the middle while preserving that
    ...
    return __internal::__except_handler([&] {
                                       ...
                                       _RandomAccessIterator __target)
        {
            ...
            {
                __internal::__brick_swap_ranges(__begin, __end, __target, _IsVector{});
            }
            else
            {
            ...
            -> _RandomAccessIterator
        {
            ...
            {
                __swap_ranges(__block_begin, __block_end, __target_region_end - __block_size);
                return __target_region_end - __block_size;
            }

            __swap_ranges(__block_begin, __block_begin + __gap, __block_end);
            ...
            -> _PartitionRange
        {
            ...
            if (__val1.__has_false_leftover())
            {
                ...
                {
                    // False leftover in __val1, true leftover (or none) in __val2
                    ...
                    if (__false_leftover_size <= __true_leftover_size)
                    {
                    ...
                    else
                    {
                        ...
                        // __val2 true leftover is fully consumed by the swap, so the merged range has none
            ...
            else if (__val1.__has_true_leftover())
            {
                ...
                {
                    // True leftover in __val1, false leftover (or none) in __val2
...
template <bool _Bounded, typename _DifferenceType1, typename _DifferenceType2, typename _DifferenceTypeOut,
...
struct _SetOpReachedPosEvaluator
{
  ...
  protected:
    ...
    __eval_reached_input_positions() const
    {
        if constexpr (_Bounded)
        {
            ...
            assert(__output_size_reached_info_opt[0].has_value());
            const _OutputSizeReachedInfo& __ri_n0 = __output_size_reached_info_opt[0].value();
  ...
  protected:
    ...
    std::optional<_OutputSizeReachedInfo> __output_size_reached_info_opt[2];

    // Reached positions in the input and output ranges
    std::optional<_SetOpReachedPosEvaluatorData> __res_data_opt;
};

template <bool _Bounded, class _IsVector, typename _ProcessingDataPointer, typename _SetRange, typename _OutputIterator,
...
struct _ParallelSetOpScanPred
{
  ...
  protected:
    ...
    _RandomAccessIterator
    __advance_clamped(_RandomAccessIterator __begin, Size __n, _RandomAccessIterator __end) const
    {
        assert(__begin <= __end);
        using _CommonSize = std::common_type_t<decltype(__end - __begin), Size>;
        return __begin + std::min<_CommonSize>(__end - __begin, __n);
    }
};
...
{
    _ParallelSetOpScanPred<_Bounded, _IsVector, _ProcessingDataPointer, _SetRange, _OutputIterator,
