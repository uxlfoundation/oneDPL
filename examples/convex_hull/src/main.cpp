...
void find_hull_points(Policy exec, Iterator first, Iterator last, pointVec_t &H,
                      point_t p1, point_t p2);

// Iteration Phase based on the divide and conquer technique
template <typename Policy, typename Iterator>
void divide_and_conquer(Policy exec, Iterator first, Iterator last,
                        pointVec_t &H, point_t p1, point_t fp, point_t p2) {
  pointVec_t H1;
  // decomposes the work and combines the results
  if (first - last >
      grain_size) {  // for small set size parallel policy could be inefficient
    find_hull_points(exec, first, last, H, p1, fp);
    find_hull_points(exec, first, last, H1, fp, p2);
  } else {
    find_hull_points(oneapi::dpl::execution::unseq, first, last, H, p1, fp);
    find_hull_points(oneapi::dpl::execution::unseq, first, last, H1, fp, p2);
  }
...
void find_hull_points(Policy exec, Iterator first, Iterator last, pointVec_t &H,
  ...
  // Find points from the range [first, last-1] that are on the right side of
  // the segment [p1,p2]
  ...
  if ((end - P_reduced.cbegin()) < 2) {
    ...
    // Find the farthest point from the segment [p1,p1], it will be in the
    // convex hull
    auto far_point = *std::max_element(
        exec, P_reduced.cbegin(), end,
    ...
    // Repeat for segments [p1, far_point] and [far_point, p2] with points from
    // [P_reduced.cbegin(), end-1]
...
#if (__INTEL_COMPILER >= 1900 && __INTEL_COMPILER <= 1910) && \
    (PSTL_VERSION >= 200 && PSTL_VERSION <= 204)
  // A workaround for incorrectly working minmax_element
  point_t p1 = *std::min_element(oneapi::dpl::execution::par_unseq,
                                 points.cbegin(), points.cend());
  point_t p2 = *std::max_element(oneapi::dpl::execution::par_unseq,
                                 points.cbegin(), points.cend());
#else
  auto minmaxx = std::minmax_element(oneapi::dpl::execution::par_unseq,
                                     points.cbegin(), points.cend());
  point_t p1 = *minmaxx.first;
  point_t p2 = *minmaxx.second;
#endif

  // Divide the set of points into two subsets, which will be processed
  // recursively
  divide_and_conquer(oneapi::dpl::execution::par_unseq, points.cbegin(),
                     points.cend(), hull, p1, p2, p1);
}

// Check if a polygon is convex
bool is_convex(const pointVec_t &points) {
  return std::all_of(oneapi::dpl::execution::par_unseq,
                     oneapi::dpl::counting_iterator<size_t>(size_t(0)),
                     oneapi::dpl::counting_iterator<size_t>(points.size()),
                     [&points](size_t i) {
                       point_t p0(points[i]);
...
int main(int argc, char *argv[]) {
  ...
  std::generate(oneapi::dpl::execution::par, points.begin(), points.end(),
                util::random_point<double>);
  std::cout << "Points were initialized. Number of the points " << points.size()
            << std::endl;
  ...
  std::cout << "Computational time "
            ...
            << "  Points in the hull: " << hull.size()
            << "  The convex hull is "
            << (is_convex(hull) ? "correct" : "incorrect") << std::endl;

  // writing the results
  std::ofstream fout(output_file);
  if (fout.is_open()) {
    for (auto p : hull) fout << p << std::endl;
    std::cout << "success" << std::endl;
    std::cout << "The convex hull has been stored to a file " << output_file
              << std::endl;
  } else {
    std::cout << "Cannot open a file " << output_file << " to store result"
              << std::endl;
  }
