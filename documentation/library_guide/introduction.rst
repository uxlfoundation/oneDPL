...
* The following algorithms require additional O(n) memory space for parallel execution:
  ...
  ``unique``, ``unique_copy``.
* With device execution policies, ``partial_sort``, ``partial_sort_copy``, and ``nth_element`` sort all the
  elements they process rather than only the ones requested, so they perform more comparisons than the
  complexity the C++ standard specifies for them. The results still satisfy the standard's requirements.
