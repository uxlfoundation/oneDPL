...
#include <algorithm>   // for std::find_first_of, std::min
...
// Compares the brick with std::find_first_of; the positions are compared as long long, so that an 8-bit
// difference type is not printed as a character
template <typename It1, typename It2>
void
check(It1 first, It1 last, It2 s_first, It2 s_last, const char* message)
{
    const auto expected = std::find_first_of(first, last, s_first, s_last, std::equal_to<>{});
    const auto result = dpl_unseq::__simd_find_first_of(first, last, s_first, s_last, std::equal_to<>{});
    EXPECT_EQ(static_cast<long long>(expected - first), static_cast<long long>(result - first), message);
}
...
test_narrow_difference_type(It1 first, std::size_t long_n2)
{
    ...
    for (const T p : positions)
    {
        ...
        check(first, last, s.begin(), s.end(), "wrong position of a match through the last element of a short range 2");

        // The second sequence with a narrow difference type as well
        const It s_first(p);
        const It s_last = s_first + T(std::min<long long>(3, n1 - p));
        check(first, last, s_first, s_last, "wrong position of a match with narrow difference types of both ranges");
    }

    // The second sequence longer than the largest block, a match at both ends of the first sequence
    for (const T p : {T(0), T(n1 / 2), T(n1 - 1)})
    {
        const std::vector<T> s = make_second(long_n2, true, p);
        check(first, last, s.begin(), s.end(), "wrong position of a match through the last element of a long range 2");
    }
    ...
    for (const std::size_t n2 : {std::size_t(1), std::size_t(3), long_n2})
    {
        ...
        check(first, last, s.begin(), s.end(), "a match found where there is none");
    }
}
