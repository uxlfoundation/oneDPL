...
main()
{
    ...
    // and automatically matches the specialization in __scan_stop_pos_type
    ...
    using oneapi::dpl::__par_backend_hetero::__scan_stop_pos_type;

    // 1. Single plain range -> scalar
    static_assert(std::is_same_v<typename __scan_stop_pos_type<R1>::_Type, SZ>,
                  "Single range must yield scalar size type");

    // 2. zip_view<R1,R2> -> tuple<SZ,SZ>
    static_assert(std::is_same_v<typename __scan_stop_pos_type<Zip2>::_Type, std::tuple<SZ, SZ>>,
                  "zip_view<R1,R2> must yield tuple<SZ,SZ>");

    // 3. zip_view<R1,R2,R3> -> tuple<SZ,SZ,SZ>
    static_assert(std::is_same_v<typename __scan_stop_pos_type<Zip3>::_Type, std::tuple<SZ, SZ, SZ>>,
                  "zip_view<R1,R2,R3> must yield tuple<SZ,SZ,SZ>");

    // 4. R1, R2 -> flat tuple<SZ,SZ> (NOT nested)
    static_assert(std::is_same_v<typename __scan_stop_pos_type<R1, R2>::_Type, std::tuple<SZ, SZ>>,
                  "Two plain ranges must yield flat tuple<SZ,SZ>");

    // 5. R1, R2, R3 -> flat tuple<SZ,SZ,SZ> (NOT nested)
    static_assert(std::is_same_v<typename __scan_stop_pos_type<R1, R2, R3>::_Type, std::tuple<SZ, SZ, SZ>>,
                  "Three plain ranges must yield flat tuple<SZ,SZ,SZ>");

    // 6. Zip2 + R3 -> zip grouped (nested), R3 flat
    static_assert(std::is_same_v<typename __scan_stop_pos_type<Zip2, R3>::_Type, std::tuple<std::tuple<SZ, SZ>, SZ>>,
                  "zip_view<R1,R2> + R3 must yield tuple<tuple<SZ,SZ>, SZ>");

    // 7. R1 + Zip2 -> R1 flat, zip nested at tail
    static_assert(std::is_same_v<typename __scan_stop_pos_type<R1, Zip2>::_Type, std::tuple<SZ, std::tuple<SZ, SZ>>>,
                  "R1 + zip_view<R1,R2> must yield tuple<SZ, tuple<SZ,SZ>>");

    // 8. Zip2 + R3 + R3 -> outer flat, zip nested at head
    static_assert(
        std::is_same_v<typename __scan_stop_pos_type<Zip2, R3, R3>::_Type, std::tuple<std::tuple<SZ, SZ>, SZ, SZ>>,
