...
DEFINE_TEST(test_remove_if)
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        auto pos = (last - first) / 2;
        auto res1 = std::remove_if(CREATE_NEW_POLICY(exec, 0), first, last, CheckState<T1, decltype(pos)>{pos});
        wait_and_throw(exec);
...
DEFINE_TEST(test_unique)
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        // invoke
        auto result_last = std::unique(CREATE_NEW_POLICY(exec, 0), first, last, TestUtils::IsEqual<IteratorValueType>{});
        wait_and_throw(exec);
...
DEFINE_TEST(test_transform_inclusive_scan)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = std::transform_inclusive_scan(CREATE_NEW_POLICY(exec, 0), first1, last1, first2, std::plus<T1>(), TransformOp<T1>{}, value);
        wait_and_throw(exec);
        ...
        auto res2 = std::transform_inclusive_scan(CREATE_NEW_POLICY(exec, 1), first1, last1, first2, std::plus<T1>(), TransformOp<T1>{});
...
DEFINE_TEST(test_transform_exclusive_scan)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = std::transform_exclusive_scan(CREATE_NEW_POLICY(exec, 2), first1, last1, first2, T1{}, std::plus<T1>(), TransformOp<T1>{});
        wait_and_throw(exec);
...
DEFINE_TEST(test_copy_if)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = std::copy_if(CREATE_NEW_POLICY(exec, 0), first1, last1, first2, TestUtils::IsGreatThan<T1>{-1});
        wait_and_throw(exec);
        ...
        auto res2 = std::copy_if(CREATE_NEW_POLICY(exec, 1), first1, last1, first2, TestUtils::IsOdd<T1>{});
        wait_and_throw(exec);
...
DEFINE_TEST(test_unique_copy)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 /* last2 */, Size n)
    {
        ...
        auto result_last = std::unique_copy(CREATE_NEW_POLICY(exec, 0), first1, last1,
