...
DEFINE_TEST(test_binary_search)
{
               ...
               Iterator3 result_first, Iterator3 /*result_last*/, Size n)
    {
        ...
        auto res1 = oneapi::dpl::binary_search(CREATE_NEW_POLICY(exec, 0), first, last, value_first, value_last, result_first);
        ...
        auto res2 = oneapi::dpl::binary_search(CREATE_NEW_POLICY(exec, 1), first, last, value_first, value_last, result_first, TestUtils::IsLess<ValueT>{});
        exec.queue().wait_and_throw();
               ...
               Iterator3 result_first, Iterator3 /*result_last*/, Size n)
    {
        ...
        auto res1 = oneapi::dpl::binary_search(CLONE_NEW_POLICY(exec), first, last, value_first, value_last, result_first);
        EXPECT_TRUE(std::distance(result_first, res1) == n, "wrong return value, host policy");
        check_and_clean(result_first, n);

        // call algorithm with comparator
        auto res2 = oneapi::dpl::binary_search(CLONE_NEW_POLICY(exec), first, last, value_first, value_last, result_first, TestUtils::IsLess<ValueT>{});
