...
DEFINE_TEST(test_lower_bound)
{
               ...
               Iterator3 result_first, Iterator3 /*result_last*/, Size n)
    {
        ...
        auto res1 = oneapi::dpl::lower_bound(CLONE_NEW_POLICY(exec), first, last, value_first, value_last, result_first);
        EXPECT_TRUE(std::distance(result_first, res1) == n, "wrong return value, host policy");
        check_and_clean(result_first, value_first, n);

        // call algorithm with comparator
        auto res2 = oneapi::dpl::lower_bound(CLONE_NEW_POLICY(exec), first, last, value_first, value_last, result_first, TestUtils::IsLess<ValueT>{});
