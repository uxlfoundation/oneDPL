...
main()
{
    ...
    // KSATODO: the device path of rotate_copy reads the input range through an access_mode::read
    // accessor, so the scalar overload of its walk functor (unseq_backend_sycl.h:885) assigns the output
    // element from a const lvalue; std::indirectly_copyable only requires *__out = *__in with *__in the
    // non-const lvalue iter_reference_t of the input iterator. Requesting read_write access for the
    // input of the walk is enough to fix it. reverse_copy above runs the same shape and is conforming.
    {
    ...
    // KSATODO: remove_copy is copy_if with a negated equality against the searched value, so its device
    // path hits the const copy of the input element described in the note above copy_if.
    {
        auto call = [](auto&& policy, auto&& in_view, auto&& out_view) {
...

    {
        ...
        // KSATODO: replace_copy stores the new value exactly as replace_copy_if does, see the note above.
    ...
    // KSATODO: the device path assigns a const copy of the input element to the output one, see the note
    // above copy_if; the projection is never reached.
    {
        auto call = [](auto&& policy, auto&& in_view, auto&& out_view) {
            return dpl_ranges::remove_copy(std::forward<decltype(policy)>(policy), in_view, out_view, search_value{3},
                                           replace_proj_mut{});
    ...
    // KSATODO: the host side of the two replacing copies stores the new value by value, see the note
    // above the const predicate calls; the copy of the new value breaks the call before the predicate,
    // respectively the projection, is ever reached.
    {
        auto call = [](auto&& policy, auto&& in_view, auto&& out_view) {
            using out_t = std::ranges::range_value_t<std::remove_cvref_t<decltype(out_view)>>;
