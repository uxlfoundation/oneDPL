...
main()
{
    ...
    // KSATODO: the host side of inplace_merge returns the sentinel of the range instead of an
    // iterator, see the note above the non-const comparator call below; that half of the gap is
    // independent of the comparator and hits this call as well. Its device path is conforming with a
    // comparator accepting const lvalues.
    {
    ...
    // partition returns the tail of the elements which do not satisfy the predicate.
    // KSATODO: the device path of partition builds the mask of the scan out of an access_mode::read
    // accessor, so __gen_mask (parallel_backend_sycl_reduce_then_scan.h:475,515, through the walk of
    // unseq_backend_sycl.h:557) hands the predicate a const lvalue, which std::indirect_unary_predicate
    // does not ask it to accept: the range of partition is mutable, so its element is only ever seen as
    // a non-const lvalue. Requesting read_write access for the range of the mask generator is enough to
    // fix it. stable_partition below reaches the very same mask generator, so it is broken in exactly
    // the same way; it needs its own macro only because the two patterns do not have to be fixed at
    // once.
    {
        auto call = [](auto&& policy, auto&& view) {
    ...
    // KSATODO: the parallel host pattern of sort does not compile. The merge step of the TBB merge
    // sort splits the two runs it merges with std::lower_bound / std::upper_bound over the element at
    // the split point (parallel_backend_tbb.h:1023,1026,1034,1037 in __merge_func::split_merging), and
    // the lookup binds that element to its const value parameter before handing it to the comparator
    // through __binary_op (utils.h:203), while std::sortable only asks the comparator to accept the
    // non-const lvalues iter_reference_t and iter_value_t& of the range. Searching the split point
    // against the element itself instead of a const value parameter is enough to fix it. Only par and
    // par_unseq reach that merge, but the macro switches all four host policies off. The device path
    // is conforming with this comparator; the one device gap of sort is its radix sort, which is
    // reached through a projected key alone, see the last family of this test.
    {
        auto call = [](auto&& policy, auto&& view) {
    ...
    // KSATODO: stable_sort reaches the very same __merge_func::split_merging as sort above, so its
    // parallel host pattern is broken in exactly the same way; it needs its own macro only because the
    // two patterns do not have to be fixed at once. Its device path is conforming with this comparator.
    {
        auto call = [](auto&& policy, auto&& view) {
    ...
    // KSATODO: the parallel host pattern of partial_sort reaches the very same
    // __merge_func::split_merging as sort and stable_sort above and is broken in exactly the same way;
    // it needs its own macro only because the three patterns do not have to be fixed at once. Its
    // device path is conforming with this comparator.
    {
        auto call = [](auto&& policy, auto&& view) {
            return dpl_ranges::partial_sort(std::forward<decltype(policy)>(policy), view, std::ranges::begin(view) + 10,
                                            permutable_comp_mut{});
    ...
    // wrong result itself, see _TEST_CPP20_RANGES_BROKEN_WRONG_RESULT_PARTIAL_SORT_COPY_PROJ1.
#if !_TEST_CPP20_RANGES_BROKEN_REQUIRES_PARTIAL_SORT_COPY
    ...
    // KSATODO: the parallel host pattern of nth_element compares the elements against the pivot with a
    // lambda which takes them by const reference (algorithm_impl.h:2841), while std::sortable only asks
    // the comparator to accept the non-const lvalue iter_reference_t of the range. Taking the argument
    // of that lambda by _Tp& is enough to fix it. Only par and par_unseq reach it, but the macro
    // switches all four host policies off; the device path is conforming.
    {
        auto call = [](auto&& policy, auto&& view) {
            return dpl_ranges::nth_element(std::forward<decltype(policy)>(policy), view, std::ranges::begin(view) + 10,
                                            permutable_comp_mut{});
    ...
    // KSATODO: inplace_merge does not compile with any policy at all, for three independent reasons:
    ...
    //    is independent of the comparator and hits any user range with a distinct sentinel type, which
    //    is why every host policy is off for the const comparator call above as well;
    ...
    //  - the device pattern reads the two sequences it merges through read-only accessors, so
    //    __rng1[...] and __rng2[...] are const lvalues by the time the comparator sees them
    //    (parallel_backend_sycl_merge.h:131 in __find_start_point, :188 in the serial merge and
    //    :358,404 where the diagonals are split), while std::sortable only asks the comparator to
    //    accept the non-const lvalue iter_reference_t of the range. Requesting read_write access for
    //    the two inputs of the merge is enough to fix that half.
    ...
    // here and all four host policies run. inplace_merge is the one exception: its host side returns
    // the sentinel of the range whatever the comparator is, see the note above its call.
    //----------------------------------------------------------------------------------------------
    ...
    // KSATODO: the sentinel return of algorithm_ranges_impl.h:848 breaks the host side of this call as
    // well, see the first reason in the note above the non-const comparator call of inplace_merge. With
    // the default comparator the const lvalues of the split point are fine, so the device path runs.
    {
        auto call = [](auto&& policy, auto&& view) {
            return dpl_ranges::inplace_merge(std::forward<decltype(policy)>(policy), view,
                                                std::ranges::begin(view) + std::ranges::size(view) / 2);
    ...
    // KSATODO: the one work group radix sort takes the address of the element with the built-in & when
    // it placement news it into its local storage, which no concept of sort asks for: std::sortable
    // needs the element movable and swappable and nothing else, and a user type is free to overload or
    // to delete operator&. Both
    // parallel_backend_sycl_radix_sort_one_wg.h:123 (new (&__values[__i]) _ValueT(__src[__idx]) of
    // __block_load, reached from :200) and :311 (new (&__exchange_lacc[__r]) _ValT(std::move(...)))
    // need std::addressof instead. The host side is conforming with a projected key.
    {
        auto call = [](auto&& policy, auto&& view) {
    ...
    // KSATODO: stable_sort is dispatched to the very same radix sort as sort above, so it takes the
    // address of the element with the built-in & as well; it needs its own macro only because the two
    // patterns do not have to be fixed at once. The host side is conforming with a projected key.
    {
        auto call = [](auto&& policy, auto&& view) {
