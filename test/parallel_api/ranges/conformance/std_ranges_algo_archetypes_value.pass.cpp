...
main()
{
    ...
    // count, contains, search_n and remove, both with const callables and with a non-const projection.
    ...
    // KSATODO: find_last forwards to find_last_if (glue_algorithm_ranges_impl.h:240), which reverses the
    // range with std::ranges::reverse_view (:203) and hands that view to find_if (:205). The device
    // pattern takes the range it walks by const lvalue (unseq_backend_sycl.h:555) and indexes it with
    // operator[] (:122, called from :557), but std::ranges::reverse_view has a const begin() only when the
    // range under it is a common_range, so a const reverse_view of a range with a distinct sentinel type
    // is not a random_access_range and std::ranges::view_interface gives it no operator[]. The
    // requires-clause asks for random_access_range and sized_range only, never for common_range.
    // Reversing a common range instead, e.g. std::ranges::subrange(begin(r), begin(r) + size(r)), is
    // enough. The host patterns walk the reversed view with its iterators and are conforming.
    {
    ...
    // search_n is constrained exactly like find: std::indirectly_comparable of the element against a
    // pointer to the value type. The storage holds distinct values, so the only run which can be found
    // is a run of length one.
    ...
    // KSATODO: the device side hits the very same const reverse_view of find_last_if as the call above.
    {
        auto call = [](auto&& policy, auto&& view) {
            using elem_t = std::ranges::range_value_t<std::remove_cvref_t<decltype(view)>>;
    ...
    // KSATODO: the device side hits the very same const reverse_view of find_last_if as the const
    // projection call above.
    {
        auto call = [](auto&& policy, auto&& view) {
            return dpl_ranges::find_last(std::forward<decltype(policy)>(policy), view, search_value{searched},
                                         search_proj_mut{});
    ...
    // search_n takes its predicate ahead of its projection, so the default std::ranges::equal_to has to
    // be spelled out to reach the projection at all.
