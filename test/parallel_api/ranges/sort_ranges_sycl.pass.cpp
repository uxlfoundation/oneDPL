...
test_impl(Policy&& exec)
{
...

    {
        ...
        sort(CREATE_NEW_POLICY(exec, 0), A); //check passing sycl buffer directly
        sort(CREATE_NEW_POLICY(exec, 1), all_view<int, sycl::access::mode::read_write>(B), std::greater<int>());
    }
...

    {
        ...
        sort(CREATE_NEW_POLICY(exec, 2), zip_view(views::all(A), views::all(B)), std::less{},
