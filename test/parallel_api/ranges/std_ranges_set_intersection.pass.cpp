...
main()
{
    ...
    test_range_algo<0, int, test_mode, mul1_t, div3_t>{get_scan_big_sz()}(dpl_ranges::set_intersection, set_intersection_checker);
    test_range_algo<1, int, test_mode, div3_t, mul1_t>{get_scan_big_sz()}(dpl_ranges::set_intersection, set_intersection_checker, std::ranges::less{}, proj);

    // Testing the cut-off with the serial implementation (less than __set_algo_cut_off)
    test_range_algo<2, int, test_mode, mul1_t, div3_t>{100}(dpl_ranges::set_intersection, set_intersection_checker, std::ranges::less{}, proj, proj);

    test_range_algo<3,  P2, test_mode, mul1_t, div3_t>{}(dpl_ranges::set_intersection, set_intersection_checker, std::ranges::less{}, &P2::x, &P2::x);
    test_range_algo<4,  P2, test_mode, mul1_t, div3_t>{}(dpl_ranges::set_intersection, set_intersection_checker, std::ranges::less{}, &P2::proj, &P2::proj);
    ...
    test_range_algo<5, int, test_mode, mul1_t, ms_t>{600}(dpl_ranges::set_intersection, set_intersection_checker);

    // Testing no intersection
    auto large_shift = [](auto&& v) { return v + 5000; };
    using ls_t = decltype(large_shift);
    test_range_algo<6, int, test_mode, mul1_t, ls_t>{1000}(dpl_ranges::set_intersection, set_intersection_checker);
    test_range_algo<7, int, test_mode, ls_t, mul1_t>{1000}(dpl_ranges::set_intersection, set_intersection_checker);
