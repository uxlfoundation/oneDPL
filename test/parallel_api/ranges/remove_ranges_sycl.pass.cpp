...
test_impl(Policy&& exec)
{
    ...
    using namespace oneapi::dpl::experimental::ranges;
    {
        ...
        in_end_n = remove(CREATE_NEW_POLICY(exec, 0), A, val1); //check passing a buffer
        in_end_n = remove(CREATE_NEW_POLICY(exec, 1), views::all(A) | views::take(in_end_n), val2); //check passing a view
    }
