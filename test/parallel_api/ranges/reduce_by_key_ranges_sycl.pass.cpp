...
test_impl(Policy&& exec)
{
    ...
    [[maybe_unused]] auto res = reduce_by_segment(CLONE_NEW_POLICY(exec), views::all_read(A), views::all_read(B),
...
// Check if a kernel name can be omitted when a compiler supports implicit names
    ...
    reduce_by_segment(CLONE_NEW_POLICY(exec), views::all_read(A), views::all_read(B), views::all_write(C), views::all_write(E));
#endif
}
