...
void test(Policy&& exec)
{
    ...
    [[maybe_unused]] auto res = reduce_by_segment(CREATE_NEW_POLICY(exec, 0), views::all_read(A), views::all_read(B),
...
// Check if a kernel name can be omitted when a compiler supports implicit names
    ...
    reduce_by_segment(CREATE_NEW_POLICY(exec, 1), views::all_read(A), views::all_read(B), views::all_write(C), views::all_write(E));
#endif
}
