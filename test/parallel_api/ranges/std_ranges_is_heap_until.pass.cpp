    ...
    test_std_ranges::for_each_in_tuple<GeneratorsT>{}([&]<class Generator, std::size_t GeneratorIdx>() {
        ...
        test_std_ranges::for_each_in_tuple<ComparatorsT>{}([&]<class Comparator, std::size_t ComparatorIdx>() {
            ...
            test_std_ranges::for_each_in_tuple<ProjectionsT>{}([&]<class Projection, std::size_t ProjectionIdx>() {
                ...
                test_std_ranges::test_range_algo<call_id, test_data_t, test_std_ranges::data_in, generator_t>{}(dpl_ranges::is_heap_until, is_heap_until_checker, comp, proj);

                if constexpr (std::is_same_v<proj_t, std::identity>)
                {
                    constexpr int call_id1 = call_id * 10;
                    test_std_ranges::test_range_algo<call_id1, test_data_t, test_std_ranges::data_in, generator_t>{}(dpl_ranges::is_heap_until, is_heap_until_checker, comp);

                    if constexpr (std::is_same_v<comp_t, std::ranges::less>)
                    {
                        test_std_ranges::test_range_algo<call_id1 + 1, test_data_t, test_std_ranges::data_in, generator_t>{}(dpl_ranges::is_heap_until, is_heap_until_checker);
                    }
                }
            });
        });
