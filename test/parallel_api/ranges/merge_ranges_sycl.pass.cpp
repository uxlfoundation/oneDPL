...
void test(Policy&& exec)
{
    ...
    using namespace oneapi::dpl::experimental::ranges;
    {
        ...
        merge(CREATE_NEW_POLICY(exec, 0), all_view(A), all_view(B), all_view<T, sycl::access::mode::write>(D));
        merge(CREATE_NEW_POLICY(exec, 1), A, B, E, std::less<T>()); //check passing sycl buffers directly
    }
