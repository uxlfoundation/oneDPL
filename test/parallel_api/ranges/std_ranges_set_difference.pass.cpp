...
main()
{
    ...
    test_range_algo<3,  P2, test_mode, div3_t, mul1_t>{}(dpl_ranges::set_difference, set_difference_checker, std::ranges::less{}, &P2::x, &P2::x);
    test_range_algo<4,  P2, test_mode, div3_t, mul1_t>{}(dpl_ranges::set_difference, set_difference_checker, std::ranges::less{}, &P2::proj, &P2::proj);

    // Testing no intersection
    auto large_shift = [](auto&& v) { return v + 5000; };
    using ls_t = decltype(large_shift);
    test_range_algo<5, int, test_mode,  mul1_t, ls_t>{1000}(dpl_ranges::set_difference, set_difference_checker);
    test_range_algo<6, int, test_mode, ls_t, mul1_t>{1000}(dpl_ranges::set_difference, set_difference_checker);
