...
test_transform_and_reduce(sycl::queue __q)
{
    ...
    device_array<int> __src(__host, __q);
    device_array<int> __dst_arr(__n, 0, __q);

    auto __policy = oneapi::dpl::execution::make_device_policy(__q);
    oneapi::dpl::transform(CLONE_TEST_POLICY_IDX(__policy, 1), oneapi::dpl::begin(__src), oneapi::dpl::end(__src),
                           oneapi::dpl::begin(__dst_arr), [](int __x) { return __x * 3; });

    std::vector<int> __expected(__n);
    std::transform(__host.begin(), __host.end(), __expected.begin(), [](int __x) { return __x * 3; });
    EXPECT_EQ_RANGES(__expected, __dst_arr.to_vector(__q), "transform into a device_array");

    const std::int64_t __expected_sum =
        std::accumulate(__expected.begin(), __expected.end(), std::int64_t(0), std::plus<std::int64_t>{});
    const std::int64_t __sum =
        oneapi::dpl::reduce(CLONE_TEST_POLICY_IDX(__policy, 2), oneapi::dpl::begin(__dst_arr),
                            oneapi::dpl::end(__dst_arr), std::int64_t(0), std::plus<std::int64_t>{});
    EXPECT_EQ(__expected_sum, __sum, "reduce over a device_array");

    // The const overloads of begin/end are the read-only path.
    const device_array<int>& __const_dst_arr = __dst_arr;
    const std::int64_t __const_sum =
        oneapi::dpl::reduce(CLONE_TEST_POLICY_IDX(__policy, 3), oneapi::dpl::begin(__const_dst_arr),
                            oneapi::dpl::end(__const_dst_arr), std::int64_t(0), std::plus<std::int64_t>{});
