...
#    endif // _ONEDPL_DEBUG_SYCL
    ...
    if constexpr (__read)
    {
        oneapi::dpl::fill(CREATE_NEW_POLICY(exec, 0), orig_out_first, orig_out_first + n, trash);
        if constexpr (__reset_read)
        {
            //Reset data if required
            oneapi::dpl::copy(CREATE_NEW_POLICY(exec, 1), expected_first, expected_first + n, orig_first);
        }

        //Run test
        oneapi::dpl::copy(CREATE_NEW_POLICY(exec, 2), first, last, copy_to_first);
...
#    endif // _ONEDPL_DEBUG_SYCL
    ...
    if constexpr (__write)
    {
        ...
        if constexpr (__check_write)
        {
            ...
            oneapi::dpl::fill(CREATE_NEW_POLICY(exec, 3), orig_first, orig_first + n, trash);
        }

        oneapi::dpl::copy(CREATE_NEW_POLICY(exec, 4), copy_from_first, copy_from_first + n, first);
        //check write if required (ignore discard iterator)
        if constexpr (__check_write)
        {
            //copy back data from original unwrapped sequence
            std::vector<T> copy_back(n);
            oneapi::dpl::copy(CREATE_NEW_POLICY(exec, 5), orig_first, orig_first + n, copy_back.begin());
...
#    endif // _ONEDPL_DEBUG_SYCL
        ...
        { // std::reverse_iterator(it)
            ...
            wrap_recurse<0, __reverses + 1, __read, __reset_read, __write, __check_write, __usable_as_perm_map,
                         __usable_as_perm_src, __is_reversible>(CREATE_NEW_POLICY(exec, 6), reversed_first, reversed_last, copy_from_first,
        ...
        { //transform_iterator(it,noop{})
            ...
            wrap_recurse<__recurse - 1, __reverses, __read, __reset_read, /*__write=*/false, __check_write,
                ...
                CREATE_NEW_POLICY(exec, 7), trans, trans + n, discard, copy_to_first, orig_first, orig_out_first, expected_first, trash,
                new_input_descr);
        }
        ...
        { //permutation_iteartor(it,noop{})
            ...
            wrap_recurse<__recurse - 1, __reverses, __read, __reset_read, __write, __check_write, __usable_as_perm_map,
                         __usable_as_perm_src, __is_reversible>(CREATE_NEW_POLICY(exec, 8), perm, perm + n, copy_from_first, copy_to_first,
        ...
        { //permutation_iterator(it,counting_iter)
            ...
            wrap_recurse<__recurse - 1, __reverses, __read, __reset_read, __write, __check_write, __usable_as_perm_map,
                         __usable_as_perm_src, __is_reversible>(CREATE_NEW_POLICY(exec, 9), perm, perm + n, copy_from_first, copy_to_first,
                                                                orig_first, orig_out_first, expected_first, trash,
                                                                new_input_descr);
        }
        ...
        { //permutation_iterator(counting_iterator,it)
            ...
            wrap_recurse<__recurse - 1, __reverses, __read, __reset_read, /*__write=*/false, __check_write,
                         __usable_as_perm_map, __usable_as_perm_src, __is_reversible>(
                CREATE_NEW_POLICY(exec, 10), perm, perm + n, discard, copy_to_first, orig_first, orig_out_first, expected_first, trash,
                new_input_descr);
        }
        ...
        { //zip_iterator(counting_iterator,it)
            ...
            wrap_recurse<__recurse - 1, __reverses, __read, __reset_read, /*__write=*/false, __check_write,
                ...
                CREATE_NEW_POLICY(exec, 11), zip, zip + n, discard, zip_out, orig_first, orig_out_first, expected_first, trash,
                new_input_descr);
        }
        ...
        { //zip_iterator(it, discard_iterator)
            ...
            wrap_recurse<__recurse - 1, __reverses, /*__read=*/false, false, __write, __check_write,
                         /*__usable_as_perm_map=*/false, __usable_as_perm_src, __is_reversible>(
                CREATE_NEW_POLICY(exec, 12), zip, zip + n, zip_in, discard, orig_first, orig_out_first, expected_first, trash,
                new_input_descr);
        }
    }
}
