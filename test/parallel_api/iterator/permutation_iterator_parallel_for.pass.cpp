...
DEFINE_TEST_PERM_IT(test_transform, PermItIndexTag)
{
    ...
    struct TestImplementation
    {
        ...
        void operator()(Policy&& exec, TPermutationIterator permItBegin, TPermutationIterator permItEnd) const
        {
            ...
            auto itResultEnd = dpl::transform(CREATE_NEW_POLICY(exec, 0), permItBegin, permItEnd, first2, TransformOp{});
            wait_and_throw(exec);
            ...
            dpl::copy(CREATE_NEW_POLICY(exec, 1), permItBegin, permItEnd, sourceData.begin());
            wait_and_throw(exec);
            std::vector<TestValueType> transformedDataResult(testing_n);
            dpl::copy(CREATE_NEW_POLICY(exec, 2), first2, itResultEnd, transformedDataResult.begin());
            wait_and_throw(exec);
