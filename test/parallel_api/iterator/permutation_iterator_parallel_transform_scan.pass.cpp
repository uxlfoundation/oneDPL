...
DEFINE_TEST_PERM_IT(test_remove_if, PermItIndexTag)
{
    ...
    struct TestImplementation
    {
        ...
        operator()(Policy&& exec, TPermutationIterator permItBegin, TPermutationIterator permItEnd) const
        {
            ...
            dpl::copy(CLONE_NEW_POLICY(exec), permItBegin, permItEnd, sourceData.begin());
            wait_and_throw(exec);

            const TestUtils::IsGreatThan<TestValueType> op{0};

            auto itEndNewRes = dpl::remove_if(CLONE_NEW_POLICY(exec), permItBegin, permItEnd, op);
            wait_and_throw(exec);
            ...
            dpl::copy(CLONE_NEW_POLICY(exec), permItBegin, itEndNewRes, resultRemoveIf.begin());
            wait_and_throw(exec);
