...
DEFINE_TEST_PERM_IT(test_remove_if, PermItIndexTag)
{
    ...
    struct TestImplementation
    {
        ...
        operator()(Policy&& exec, TPermutationIterator permItBegin, TPermutationIterator permItEnd) const
        {
            ...
            auto itEndNewRes = dpl::remove_if(CREATE_NEW_POLICY(exec, 1), permItBegin, permItEnd, op);
            wait_and_throw(exec);
            ...
            dpl::copy(CREATE_NEW_POLICY(exec, 2), permItBegin, itEndNewRes, resultRemoveIf.begin());
            wait_and_throw(exec);
