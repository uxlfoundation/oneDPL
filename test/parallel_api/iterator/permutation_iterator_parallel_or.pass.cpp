...
DEFINE_TEST_PERM_IT(test_is_heap, PermItIndexTag)
{
    ...
    struct TestImplementation
    {
        ...
        void operator()(Policy&& exec, TPermutationIterator permItBegin, TPermutationIterator permItEnd) const
        {
            ...
            const auto resultIsHeap = dpl::is_heap(CLONE_NEW_POLICY(exec), permItBegin, permItEnd);
            wait_and_throw(exec);

            // Copy data back
            std::vector<TestValueType> expected(testing_n);
            dpl::copy(CLONE_NEW_POLICY(exec), permItBegin, permItEnd, expected.begin());
            wait_and_throw(exec);
        ...
        if constexpr (is_base_of_iterator_category_v<::std::random_access_iterator_tag, Iterator1>)
        {
            for (bool bCallMakeHeap : {false, true})
            {
                ...
                test_through_permutation_iterator<Iterator1, Size, PermItIndexTag>{first1, n}(
                    CLONE_NEW_POLICY(exec), TestImplementation<Size>{n});
            }
        }
    }
};
