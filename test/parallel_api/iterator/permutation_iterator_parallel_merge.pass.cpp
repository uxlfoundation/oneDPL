...
DEFINE_TEST_PERM_IT(test_merge, PermItIndexTag)
{
    ...
    struct TestImlementationLevel1
    {
        ...
        void operator()(Policy&& exec, TPermutationIteratorArg permItBegin2, TPermutationIteratorArg permItEnd2) const
        {
            ...
            dpl::sort(CREATE_NEW_POLICY(exec, 0), permItBegin2, permItEnd2);
            wait_and_throw(exec);

            const auto resultEnd = dpl::merge(CREATE_NEW_POLICY(exec, 1), permItBegin1, permItEnd1, permItBegin2, permItEnd2, first3);
            wait_and_throw(exec);
            const auto resultSize = resultEnd - first3;

            // Copy data back
            std::vector<TestValueType> srcData2(testing_n2);
            dpl::copy(CREATE_NEW_POLICY(exec, 2), permItBegin2, permItEnd2, srcData2.begin());
            wait_and_throw(exec);

            std::vector<TestValueType> mergedDataResult(resultSize);
            dpl::copy(CREATE_NEW_POLICY(exec, 3), first3, resultEnd, mergedDataResult.begin());
            wait_and_throw(exec);
    ...
    struct TestImlementationLevel0
    {
        ...
        void operator()(Policy&& exec, TPermutationIterator permItBegin1, TPermutationIterator permItEnd1) const
        {
            ...
            test_through_permutation_iterator<Iterator1, Size, PermItIndexTag>{first1, n}(
                std::forward<Policy>(exec), TestImlementationLevel1<Size, Iterator3, TPermutationIterator>{
                    ...
                    Iterator3 last3, Size n)
    {
        if constexpr (is_base_of_iterator_category_v<std::random_access_iterator_tag, Iterator1>)
        {
            ...
            test_through_permutation_iterator<Iterator1, Size, PermItIndexTag>{first1, n}(
                std::forward<Policy>(exec), TestImlementationLevel0<Size, Iterator1, Iterator3>{n, first1, first3});
        }
    }
};
