    ...
    struct TestImplementation
    {
        ...
        operator()(Policy&& exec, TIterator permItBegin, TIterator permItEnd) const
        {
            ...
            if (testing_n >= 2)
            {
                ...
                dpl::copy(CLONE_NEW_POLICY(exec), permItBegin + 1, permItBegin + 2, valueToFind.begin());
                wait_and_throw(exec);

                const auto result = dpl::find(CLONE_NEW_POLICY(exec), permItBegin, permItEnd, valueToFind[0]);
                wait_and_throw(exec);
                ...
                dpl::copy(CLONE_NEW_POLICY(exec), result, result + 1, foundVal.begin());
                wait_and_throw(exec);
