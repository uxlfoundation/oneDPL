    ...
    struct TestImplementation
    {
        ...
        operator()(Policy&& exec, TIterator permItBegin, TIterator permItEnd) const
        {
            ...
            if (testing_n >= 2)
            {
                ...
                dpl::copy(CREATE_NEW_POLICY(exec, 0), permItBegin + 1, permItBegin + 2, valueToFind.begin());
                wait_and_throw(exec);

                const auto result = dpl::find(CREATE_NEW_POLICY(exec, 1), permItBegin, permItEnd, valueToFind[0]);
                wait_and_throw(exec);
                ...
                dpl::copy(CREATE_NEW_POLICY(exec, 2), result, result + 1, foundVal.begin());
                wait_and_throw(exec);
