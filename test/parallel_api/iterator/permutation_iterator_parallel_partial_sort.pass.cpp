...
DEFINE_TEST_PERM_IT(test_partial_sort, PermItIndexTag)
{
    ...
    struct TestImplementation
    {
        ...
        operator()(Policy&& exec, TPermutationIterator permItBegin, TPermutationIterator permItEnd) const
        {
            ...
            {
                dpl::partial_sort(CLONE_NEW_POLICY(exec), permItBegin, permItBegin + p, permItEnd);
                wait_and_throw(exec);

                // Copy data back
                std::vector<TestValueType> partialSortResult(p);
                dpl::copy(CLONE_NEW_POLICY(exec), permItBegin, permItBegin + p, partialSortResult.begin());
                wait_and_throw(exec);
