                            ...
                            Iterator /* expected_last */, Size n)
{
    ...
    for (int loop_stride : {-1, 1, 10, -5})
    {
        ...
        std::experimental::for_loop_n_strided(CLONE_NEW_POLICY(exec), Ssize(0), Ssize(n), loop_stride,
        ...
        std::experimental::for_loop_strided(
            CLONE_NEW_POLICY(exec), new_first, new_last, loop_stride, std::experimental::induction(lval_ind),
...
struct test_body
{
    ...
    {
        test_body_induction(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n);
        test_body_induction_strided(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n);
    }
};
