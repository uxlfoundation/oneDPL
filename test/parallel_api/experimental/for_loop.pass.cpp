                   ...
                   Size n)
{
    ...
    ::std::experimental::for_loop(CLONE_NEW_POLICY(exec), first, last, [&flip](Iterator iter) { flip(*iter); });

    ::std::for_each(expected_first, expected_last, flip);
    EXPECT_EQ_N(expected_first, first, n, "wrong effect from for_loop");

    ::std::experimental::for_loop_n(CLONE_NEW_POLICY(exec), first, n, [&flip](Iterator iter) { flip(*iter); });
                            ...
                            Iterator expected_last, Size n)
{
    ...
    ::std::experimental::for_loop(CLONE_NEW_POLICY(exec), Size(0), n, [&flip, first](Size idx) {
        auto iter = first;
    ...
    ::std::experimental::for_loop_n(CLONE_NEW_POLICY(exec), Size(0), n, [&flip, first](Size idx) {
        auto iter = first;
        ::std::advance(iter, idx);
        flip(*iter);
    });
...
struct test_for_loop_impl
{
    ...
    {
        test_body_for_loop(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n);
        test_body_for_loop_integral(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n);
    }
};
...
struct test_for_loop_strided_impl
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Iterator expected_first, Iterator expected_last, Size n,
               size_t stride)
    {
        test_body_for_loop_strided(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n, stride);
        test_body_for_loop_strided_n(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n, stride);

        test_body_for_loop_strided_integral(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n, stride);

        test_body_for_loop_strided_n_integral(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n, (long)stride);

        // Additionally check negative stride with integral and iterator sequence.
        test_body_for_loop_strided_n_integral(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n, -(long)stride);
        test_body_for_loop_strided_neg(CLONE_NEW_POLICY(exec), first, last, expected_first, expected_last, n, -(long)stride);
    }
};
