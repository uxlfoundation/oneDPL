...
void
test_with_buffers(Policy&& policy)
{
    const int n = 100;
    ...
    auto alpha = dpl::experimental::transform_inclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan1),
        dpl::begin(x), dpl::end(x), dpl::begin(y), std::plus<int>(), [](auto x) { return x * 10; });
    ...
    auto fut1b = dpl::experimental::transform_inclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan2),
        dpl::begin(x), dpl::end(x), dpl::begin(y), std::plus<int>(), [](auto x) { return x * 10; }, 1);
    ...
    auto beta = dpl::experimental::transform_exclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan3),
        dpl::begin(x), dpl::end(x), dpl::begin(y), 0, std::plus<int>(), [](auto x) { return x * 10; });
    ...
    auto gamma = dpl::experimental::inclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan4),
    ...
    auto fut3b = dpl::experimental::inclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan5),
    ...
    auto fut3c = dpl::experimental::inclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan6),
    ...
    auto delta = dpl::experimental::exclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan7),
    ...
    auto fut4b = dpl::experimental::exclusive_scan_async(
        CREATE_NEW_POLICY_WITH_NAME(policy, class Scan8),
...
int main()
{
    ...
    TestUtils::check_compile([](auto&& policy) { test_with_buffers(std::forward<decltype(policy)>(policy)); });
