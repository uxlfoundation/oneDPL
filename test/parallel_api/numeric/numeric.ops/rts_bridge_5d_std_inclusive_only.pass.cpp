// -*- C++ -*-
//===-- rts_bridge_5d_std_inclusive_only.pass.cpp ---------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 5d: Same as 5c but ONLY inclusive_scan (no exclusive_scan).
// Disambiguates whether 5c's crash is from inclusive_scan alone, or from
...
int run_test() {
    ...
    std::printf("[bridge5d] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());

    std::printf("[bridge5d] Calling std::inclusive_scan ONLY with device_policy\n");

    auto result = std::inclusive_scan(policy, h_input.begin(), h_input.end(), h_out.begin());

    if (result != h_out.begin() + N) {
        std::printf("[bridge5d] ERROR: returned iterator is wrong\n");
        return 1;
    }
    ...
    for (std::size_t i = 0; i < N; i++) {
        if (h_out[i] != h_expected[i]) {
            if (errors < 20)
                std::printf("[bridge5d] MISMATCH [%zu]: got %d expected %d\n", i, h_out[i], h_expected[i]);
            errors++;
        }
    }
    std::printf("[bridge5d] %s: %d errors out of %zu\n", errors ? "FAIL" : "PASS", errors, N);
