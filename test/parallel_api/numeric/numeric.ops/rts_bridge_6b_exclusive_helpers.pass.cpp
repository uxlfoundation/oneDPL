// -*- C++ -*-
//===-- rts_bridge_6b_exclusive_helpers.pass.cpp ----------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 6b: Uses the actual oneDPL scan helper functions
// (__shift_group_right, __group_broadcast, __sub_group_scan,
// __scan_through_elements_helper) but with __is_inclusive = false (exclusive
// scan). Based on rts_bridge_3_onedpl_helpers.pass.cpp.
// Key changes from bridge 3:
//   - __is_inclusive = false
//   - First block / first group / first subgroup: sgc.__setup(T{0}), sgci = true
...
int run_test() {
    ...
    std::printf("[bridge6b] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        // ============ REDUCE KERNEL ============
        // Reduce phase is always inclusive — only accumulates carries.
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ReduceK6b>(ndr, [=](sycl::nd_item<1> ndi) {
                ...
                lazy<T> carry;
                if (sid < asg) {
                    // Reduce phase always uses inclusive scan (same as bridge 3)
                    rts::__scan_through_elements_helper<__use_subgroup_ops, /*__is_inclusive=*/true,
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanK6b>(ndr, [=](sycl::nd_item<1> ndi) {
                auto sg = ndi.get_sub_group();
                std::uint8_t ssz = sg.get_max_local_range()[0];
                std::uint8_t slid = sg.get_local_linear_id();
                std::uint32_t sid = sg.get_group_linear_id();
                ...
                // Step 1: load sub-group partials (same as bridge 3)
                if (sid == 0) {
                    std::uint8_t it = static_cast<std::uint8_t>(ceiling_div(asg, (std::uint32_t)ssz));
                ...
                // EXCLUSIVE: block 0, group 0, sg 0 gets carry = init (T{0})
                if (cb == 0) {
                    if (sid > 0) sgc.__setup(sg_parts[std::min(sid - 1, asg - 1)]);
                    else if (gid > 0) sgc.__setup(sg_parts[asg]);
                    else { sgc.__setup(T{0}); sgci = true; }  // EXCLUSIVE: init value
                } else {
                ...
                // Step 3: scan through elements using oneDPL helper
                // EXCLUSIVE: __is_inclusive = false
                std::size_t sgs = gs + (sid * ips);
                std::size_t stid = sgs + slid;
                ...
                if (sgci) {
                    rts::__scan_through_elements_helper<__use_subgroup_ops, __is_inclusive,
                        ...
                        in_rng, out_rng, stid, cn, lip, sgs, sid, asg, sl);
                } else {
                    rts::__scan_through_elements_helper<__use_subgroup_ops, __is_inclusive,
    ...
    for (std::size_t i = 0; i < N; i++) {
        if (h_out[i] != h_expected[i]) {
            if (errors < 20)
                std::printf("[bridge6b] MISMATCH [%zu]: got %d expected %d\n", i, h_out[i], h_expected[i]);
            errors++;
        }
    }
    std::printf("[bridge6b] %s: %d errors out of %zu\n", errors ? "FAIL" : "PASS", errors, N);
