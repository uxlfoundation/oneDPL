...
//
//===----------------------------------------------------------------------===//
// Isolation test: Skip the reduce kernel, only run the scan kernel.
// The output will be garbage (scratch memory uninitialized), but we're
// testing whether the crash comes from the reduce kernel or the scan kernel.
// If this passes: the reduce kernel is the crash source.
// If this crashes: the scan kernel is the crash source (or both).

#define _ONEDPL_RTS_SKIP_REDUCE
