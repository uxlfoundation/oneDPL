// -*- C++ -*-
//===-- rts_bridge_3_onedpl_helpers.pass.cpp -------------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 3: Uses the actual oneDPL scan helper functions
// (__shift_group_right, __group_broadcast, __sub_group_scan,
// __scan_through_elements_helper) but still with USM pointers (not range
// objects) and manual kernel submission (not the submitter structs).
// Tests whether the oneDPL helper template chain triggers the crash.
...
int run_test() {
    ...
    std::printf("[bridge3] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ReduceK3>(ndr, [=](sycl::nd_item<1> ndi) {
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanK3>(ndr, [=](sycl::nd_item<1> ndi) {
                auto sg = ndi.get_sub_group();
                std::uint8_t ssz = sg.get_max_local_range()[0];
                std::uint8_t slid = sg.get_local_linear_id();
                std::uint32_t sid = sg.get_group_linear_id();
    ...
    for (std::size_t i = 0; i < N; i++) {
        if (h_out[i] != h_expected[i]) {
            if (errors < 20)
                std::printf("[bridge3] MISMATCH [%zu]: got %d expected %d\n", i, h_out[i], h_expected[i]);
            errors++;
        }
    }
    std::printf("[bridge3] %s: %d errors out of %zu\n", errors ? "FAIL" : "PASS", errors, N);
