// -*- C++ -*-
//===-- rts_bridge_mat_3n_no_unroll.pass.cpp ------------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 3n (Matrix2x2): Same as 3e but with _ONEDPL_PRAGMA_UNROLL
// defined as empty (no-op) before including oneDPL headers. If this passes
// while 3e fails, the pragma unroll is the trigger for the crash.
...
// Override _ONEDPL_PRAGMA_UNROLL to be empty AFTER onedpl_config.h has defined it
// but BEFORE the RTS headers use it.
#undef _ONEDPL_PRAGMA_UNROLL
...
// Kernel name tags
struct Bridge3nReduceKernel;
struct Bridge3nScanKernel;
...
int run_test() {
    ...
    std::printf("[bridge3n_mat] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge3n_mat] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u use_slm=%d\n",
    ...
    using _ReduceKernel = oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<Bridge3nReduceKernel>;
    using _ScanKernel = oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<Bridge3nScanKernel>;
