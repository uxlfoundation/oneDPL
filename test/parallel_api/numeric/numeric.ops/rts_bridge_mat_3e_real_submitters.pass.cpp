// -*- C++ -*-
//===-- rts_bridge_mat_3e_real_submitters.pass.cpp ---------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 3e (Matrix2x2): Uses the REAL submitter types
// (__parallel_reduce_then_scan_reduce_submitter / scan_submitter) directly,
// with our own parameter calculation. This tests whether the real submitter
// template code generation (including reqd_sub_group_size(32), dual-path
// instantiation, etc.) is the cause of the crash.
...
// Kernel name tags
struct Bridge3eReduceKernel;
struct Bridge3eScanKernel;
...
int run_test() {
    ...
    std::printf("[bridge3e_mat] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge3e_mat] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u use_slm=%d\n",
    ...
    using _ReduceKernel = oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<Bridge3eReduceKernel>;
    using _ScanKernel = oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<Bridge3eScanKernel>;
