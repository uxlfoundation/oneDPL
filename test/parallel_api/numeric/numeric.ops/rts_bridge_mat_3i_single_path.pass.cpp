// -*- C++ -*-
//===-- rts_bridge_mat_3i_single_path.pass.cpp ----------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 3i (Matrix2x2): Same as 3e (real submitter types) but with
// _ONEDPL_RTS_OVERRIDE_USE_SLM=1 to force the SLM path at the host level.
// On CPU, the SLM path is already the one taken at runtime, but forcing it
// may allow the compiler to constant-propagate and eliminate the dead
// body<true> (native sub-group ops) path. Tests whether dual-path compilation
// causes register pressure / vectorization issues.

#define _ONEDPL_REDUCE_THEN_SCAN_DEBUG 0
#define _ONEDPL_RTS_OVERRIDE_USE_SLM 1
...
// Kernel name tags
struct Bridge3iReduceKernel;
struct Bridge3iScanKernel;
...
int run_test() {
    ...
    std::printf("[bridge3i_mat] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    std::printf("[bridge3i_mat] _ONEDPL_RTS_OVERRIDE_USE_SLM=1 (single path forced)\n");
    ...
    const bool __use_slm_for_comm = true; // forced by _ONEDPL_RTS_OVERRIDE_USE_SLM

    std::printf("[bridge3i_mat] wg_size=%u num_wg=%u ipi=%u blocks=%zu use_slm=%d\n",
    ...
    using _ReduceKernel = oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<Bridge3iReduceKernel>;
    using _ScanKernel = oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<Bridge3iScanKernel>;
