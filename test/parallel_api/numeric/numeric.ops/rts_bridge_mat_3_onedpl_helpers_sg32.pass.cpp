// -*- C++ -*-
//===-- rts_bridge_mat_3_onedpl_helpers.pass.cpp ----------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 3 (Matrix2x2): Uses the actual oneDPL scan helper functions
// (__scan_through_elements_helper, __sub_group_scan, __sub_group_scan_partial)
// with Matrix2x2<int32_t> and multiply_matrix to exercise the SLM path
// (non-trivially-copyable type) with a non-commutative binary op.
...
int run_test() {
    ...
    constexpr bool __use_subgroup_ops = false; // SLM path (required for non-trivially-copyable)
    ...
    std::printf("[bridge3_mat_sg32] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge3_mat_sg32] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ReduceK3MatSg32>(ndr, [=](sycl::nd_item<1> ndi) [[sycl::reqd_sub_group_size(32)]] {
                ...
                lazy<T> carry;
                if (sid < asg) {
                    // Use oneDPL's __scan_through_elements_helper
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanK3MatSg32>(ndr, [=](sycl::nd_item<1> ndi) [[sycl::reqd_sub_group_size(32)]] {
                auto sg = ndi.get_sub_group();
                std::uint8_t ssz = sg.get_max_local_range()[0];
                std::uint8_t slid = sg.get_local_linear_id();
                std::uint32_t sid = sg.get_group_linear_id();
