// -*- C++ -*-
//===-- rts_bridge_1_lazy_carry.pass.cpp -----------------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 1: Same as standalone_rts_reproducer but replaces plain T carry
// variables with __lazy_ctor_storage<T> (oneDPL's union-based deferred-init
// type). Tests whether the lazy storage abstraction triggers the crash.
...
int run_test() {
    ...
    std::printf("[bridge1] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            std::size_t cir = ir;
            std::size_t cb = b;
            cgh.parallel_for<ReduceK1>(ndr, [=](sycl::nd_item<1> ndi) {
                ...
                bool ci = false;
                if (sid < asg) {
                    scan_through_reduce(sg, std::plus<T>{}, carry, ci, d_in, stid, cn, lip, sgs, sid, asg, sl);
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanK1>(ndr, [=](sycl::nd_item<1> ndi) {
                auto sg = ndi.get_sub_group();
                std::uint8_t ssz = sg.get_max_local_range()[0];
                std::uint8_t slid = sg.get_local_linear_id();
                std::uint32_t sid = sg.get_group_linear_id();
                ...
                if (cb == 0) {
                    ...
                    if (sid > 0)
                        sgc.__setup(std::plus<T>{}(bci, sg_parts[std::min(sid - 1, asg - 1)]));
                    else if (gid > 0)
                        sgc.__setup(std::plus<T>{}(bci, sg_parts[asg]));
                    else
                        sgc.__setup(bci);
                }
                ...
                scan_through_scan(sg, std::plus<T>{}, sgc, sgci, d_in, d_out, stid, cn, lip, sgs, sid, asg, sl);
    ...
    for (std::size_t i = 0; i < N; i++) {
        if (h_out[i] != h_expected[i]) {
            if (errors < 20)
                std::printf("[bridge1] MISMATCH [%zu]: got %d expected %d\n", i, h_out[i], h_expected[i]);
            errors++;
        }
    }
    std::printf("[bridge1] %s: %d errors out of %zu\n", errors ? "FAIL" : "PASS", errors, N);
