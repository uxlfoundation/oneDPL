// -*- C++ -*-
//===-- rts_bridge_mat_6b_exclusive_helpers.pass.cpp ----------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test mat_6b: Matrix2x2 version of bridge 6b. Uses the actual oneDPL
// scan helper functions with __is_inclusive = false (exclusive scan) and
...
int run_test() {
    ...
    std::printf("[bridge_mat_6b] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge_mat_6b] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    // Initialize output and scratch with identity matrices
    std::vector<T> h_out_init(N);
    std::vector<T> h_scratch_init(scratch_size);
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ReduceKMat6b>(ndr, [=](sycl::nd_item<1> ndi) {
                ...
                lazy<T> carry;
                if (sid < asg) {
                    // Reduce phase always uses inclusive scan
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanKMat6b>(ndr, [=](sycl::nd_item<1> ndi) {
                auto sg = ndi.get_sub_group();
                std::uint8_t ssz = sg.get_max_local_range()[0];
                std::uint8_t slid = sg.get_local_linear_id();
                std::uint32_t sid = sg.get_group_linear_id();
                ...
                // EXCLUSIVE: block 0, group 0, sg 0 gets carry = init (identity matrix)
                if (cb == 0) {
                    if (sid > 0) sgc.__setup(sg_parts[std::min(sid - 1, asg - 1)]);
                    else if (gid > 0) sgc.__setup(sg_parts[asg]);
                    else { sgc.__setup(T()); sgci = true; }  // EXCLUSIVE: init value (identity)
                } else {
