// -*- C++ -*-
//===-- rts_bridge_mat_5d_std_inclusive_only.pass.cpp ----------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test mat_5d: Matrix2x2 version of bridge 5d. Calls std::inclusive_scan
...
int run_test() {
    ...
    std::printf("[bridge_mat_5d] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    std::printf("[bridge_mat_5d] Calling std::inclusive_scan ONLY with device_policy\n");

    auto result = std::inclusive_scan(policy, h_input.begin(), h_input.end(), h_out.begin(), op);

    if (result != h_out.begin() + N) {
        std::printf("[bridge_mat_5d] ERROR: returned iterator is wrong\n");
        return 1;
    }
