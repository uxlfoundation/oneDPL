// -*- C++ -*-
//===-- rts_bridge_mat_1_lazy_carry.pass.cpp --------------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 1 (Matrix2x2): Same as rts_bridge_1_lazy_carry but uses
...
// Pull in oneDPL's __lazy_ctor_storage
...
// ---------- scan helpers using __lazy_ctor_storage for carry ----------
...
template <typename T, typename BinaryOp>
void scan_through_reduce(sycl::sub_group sg, BinaryOp op, lazy<T>& carry, bool& carry_init,
...
template <typename T, typename BinaryOp>
void scan_through_scan(sycl::sub_group sg, BinaryOp op, lazy<T>& carry, bool init_present,
    ...
    if (full) {
        T v = input[start_id];
        sg_scan(sg, v, op, init_present, carry, slm);
        output[start_id] = v;
        for (std::uint32_t j = 1; j < iters; j++) {
            v = input[start_id + j * sz];
            sg_scan(sg, v, op, true, carry, slm);
            output[start_id + j * sz] = v;
        }
    } else if (sg_id < active_sg) {
        std::uint32_t it = ceiling_div(static_cast<std::uint32_t>(n - sg_start), (std::uint32_t)sz);
        if (it == 1) {
            std::size_t lid = (start_id < n) ? start_id : n - 1;
            T v = input[lid];
            sg_scan_partial(sg, v, op, init_present, carry, static_cast<std::uint32_t>(n - sg_start), slm);
            if (start_id < n) output[start_id] = v;
        } else {
            T v = input[start_id];
            sg_scan(sg, v, op, init_present, carry, slm);
            output[start_id] = v;
            ...
            sg_scan_partial(sg, v, op, true, carry,
                static_cast<std::uint32_t>(n - (sg_start + (it - 1) * sz)), slm);
            if (off < n) output[off] = v;
        }
    }
}

// ---------- kernel names ----------
class ReduceK1MatSg32;
class ScanK1MatSg32;
...
int run_test() {
    ...
    std::printf("[bridge1_mat_sg32] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge1_mat_sg32] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            std::size_t cir = ir;
            std::size_t cb = b;
            cgh.parallel_for<ReduceK1MatSg32>(ndr, [=](sycl::nd_item<1> ndi) [[sycl::reqd_sub_group_size(32)]] {
                ...
                BinaryOp binary_op{};
                if (sid < asg) {
                    scan_through_reduce(sg, binary_op, carry, ci, d_in, stid, cn, lip, sgs, sid, asg, sl);
                ...
                if (sid == 0) {
                    ...
                    if (it == 1) {
                        ...
                        T v = sg_parts[li];
                        sg_scan_partial(sg, v, binary_op, false, wc, asg, sl);
                        if (slid < asg) d_scratch[ss + slid] = v;
                    } else {
                        std::uint32_t ri = slid;
                        T v = sg_parts[ri];
                        sg_scan(sg, v, binary_op, false, wc, sl);
                        d_scratch[ss + ri] = v;
                        ri += ssz;
                        for (std::uint32_t i = 1; i < (std::uint32_t)it - 1; i++) {
                            v = sg_parts[ri];
                            sg_scan(sg, v, binary_op, true, wc, sl);
                            d_scratch[ss + ri] = v;
                            ri += ssz;
                        }
                        std::uint32_t li = std::min(ri, nsl - 1);
                        v = sg_parts[li];
                        sg_scan_partial(sg, v, binary_op, true, wc, asg - ((it - 1) * ssz), sl);
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanK1MatSg32>(ndr, [=](sycl::nd_item<1> ndi) [[sycl::reqd_sub_group_size(32)]] {
                auto sg = ndi.get_sub_group();
                std::uint8_t ssz = sg.get_max_local_range()[0];
                std::uint8_t slid = sg.get_local_linear_id();
                std::uint32_t sid = sg.get_group_linear_id();
                ...
                if (sid == 0) {
                    ...
                    if (gid > 0) {
                        ...
                        if (pci == 1) {
                            ...
                            T val = d_scratch[rid];
                            sg_scan_partial(sg, val, binary_op, false, carry_last, static_cast<std::uint32_t>(etp), sl);
                        } else {
                            std::uint32_t rid = nsl * slid + off;
                            std::uint32_t rinc = nsl * ssz;
                            T val = d_scratch[rid];
                            sg_scan(sg, val, binary_op, false, carry_last, sl);
                            rid += rinc;
                            for (std::uint32_t i = 1; i < (std::uint32_t)pci - 1; i++) {
                                val = d_scratch[rid];
                                sg_scan(sg, val, binary_op, true, carry_last, sl);
                                rid += rinc;
                            }
                            std::size_t rem = etp - ((pci - 1) * ssz);
                            std::size_t frid = std::min((std::size_t)rid, sbm - 1);
                            val = d_scratch[frid];
                            sg_scan_partial(sg, val, binary_op, true, carry_last, static_cast<std::uint32_t>(rem), sl);
                        }
                ...
                if (cb == 0) {
                    if (sid > 0) {
                        sgc.__setup(sg_parts[std::min(sid - 1, asg - 1)]);
                    } else if (gid > 0) {
                        sgc.__setup(sg_parts[asg]);
                    } else {
                        sgci = false;
                    }
                } else {
                    ...
                    if (sid > 0)
                        sgc.__setup(binary_op(bci, sg_parts[std::min(sid - 1, asg - 1)]));
                    else if (gid > 0)
                        sgc.__setup(binary_op(bci, sg_parts[asg]));
                    else
                        sgc.__setup(bci);
                }
                ...
                scan_through_scan(sg, binary_op, sgc, sgci, d_in, d_out, stid, cn, lip, sgs, sid, asg, sl);

                if (slid == 0 && (ag == gid + 1) && (asg == sid + 1)) {
                    if (cb + 1 == cnb)
                        d_result[0] = sgc.__v;
                    else
                        d_scratch[cmsg + 1 - (cb % 2)] = sgc.__v;
                }
                sgc.__destroy();
            });
        });
