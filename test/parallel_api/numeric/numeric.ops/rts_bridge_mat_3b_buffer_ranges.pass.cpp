...
struct reduce_submitter_3b
{
           ...
           const std::size_t __inputs_remaining, const std::size_t __block_num) const
    {
        ...
        cgh.parallel_for<ReduceK3bMat>(__nd_range, [=, *this](sycl::nd_item<1> ndi) {
...
struct scan_submitter_3b
{
                ...
                const std::size_t __inputs_remaining, const std::size_t __block_num) const
    {
        ...
        cgh.parallel_for<ScanK3bMat>(__nd_range, [=, *this](sycl::nd_item<1> ndi) {
            auto sg = ndi.get_sub_group();
            std::uint8_t ssz = sg.get_max_local_range()[0];
            std::uint8_t slid = sg.get_local_linear_id();
            std::uint32_t sid = sg.get_group_linear_id();
...
int run_test() {
    ...
    std::printf("[bridge3b_mat] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge3b_mat] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
