...
//
//===----------------------------------------------------------------------===//
// Isolation test: Matrix2x2 scan using only host (TBB/serial) policies.
// No SYCL backend. Rules out the SYCL runtime entirely.
...
main()
{
    ...
    for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        // Host-only policies: TBB and serial — no SYCL
        invoke_on_all_host_policies()(test_inclusive_scan_matrix<T>(), in.begin(), in.end(), out.begin(),
                                      out.end(), expected.begin(), expected.end(), in.size(), init, binary_op, trash);
#if !TEST_GCC10_EXCLUSIVE_SCAN_BROKEN
        invoke_on_all_host_policies()(test_exclusive_scan_matrix<T>(), in.begin(), in.end(), out.begin(),
                                      out.end(), expected.begin(), expected.end(), in.size(), init, binary_op, trash);
#endif
    }
