// -*- C++ -*-
//===-- rts_bridge_6a_exclusive_standalone.pass.cpp -------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 6a: Standalone SYCL exclusive scan reproducer.
// Based on standalone_rts_reproducer.pass.cpp but modified for exclusive scan.
// The inclusive scan kernels are reused, but the sub-group scan functions are
// modified with an is_inclusive parameter: when false, after the scan loop,
// values are shifted right by 1 within the sub-group and lid==0 gets the
// carry-in. For block 0 / group 0 / subgroup 0, the carry is initialized to
// init (0) instead of being left uninitialized.
...
// ---------- sub-group scans with is_inclusive parameter ----------
// When is_inclusive==false: after the inclusive scan loop, shift right by 1
// and lid==0 gets the carry-in value. The carry-out is still the last
// element's inclusive scan result (same as inclusive case).

template <typename T, typename BinaryOp>
void sg_scan(sycl::sub_group sg, T& value, BinaryOp op,
             bool init_present, T& carry, bool is_inclusive, T* slm) {
    ...
    if (!is_inclusive) {
        // Save the carry-in before we modify it
        T carry_before = carry;
        // Update carry-out: last element's inclusive result
        if (init_present) {
            // The inclusive result with carry applied
            T inclusive_with_carry = op(carry, value);
            carry = slm_group_broadcast(sg, inclusive_with_carry, broadcast_id, slm);
            // Shift right by 1 for exclusive scan
            value = slm_shift_group_right(sg, inclusive_with_carry, 1, slm);
            if (lid == 0)
                value = carry_before;
        } else {
            carry = slm_group_broadcast(sg, value, broadcast_id, slm);
            // For exclusive scan without init: this shouldn't happen for the
            // output path, but we handle it for completeness in reduce phase.
            // The carry is still the inclusive scan of the last element.
            T inclusive_val = value;
            value = slm_shift_group_right(sg, inclusive_val, 1, slm);
            // lid==0 has no predecessor — value is undefined but won't be written
            // in the exclusive case because init is always present for exclusive scan.
        }
    } else {
        if (init_present) {
...
void sg_scan_partial(sycl::sub_group sg, T& value, BinaryOp op,
                     bool init_present, T& carry, std::uint32_t count,
                     bool is_inclusive, T* slm) {
    std::uint8_t lid = sg.get_local_linear_id();
    std::uint8_t sg_size = sg.get_max_local_range()[0];
...
// ---------- Inclusive-only sub-group scans (used in reduce phase and
//            inter-work-group prefix scan — these always stay inclusive) ----------
...
// Reduce phase always uses inclusive scan — it only accumulates carries.
...
// ---------- scan_through_elements: scan phase (writes output, exclusive) ----------
// Uses sg_scan / sg_scan_partial with is_inclusive=false.

template <typename T, typename BinaryOp>
void scan_through_scan_exclusive(sycl::sub_group sg, BinaryOp op, T& carry, bool init_present,
                       const T* input, T* output, std::size_t start_id, std::size_t n,
                       std::uint32_t iters_per_item, std::size_t subgroup_start_id,
                       std::uint32_t sg_id, std::uint32_t active_subgroups, T* slm) {
    std::uint8_t sg_size = sg.get_max_local_range()[0];
    bool is_full_thread = (subgroup_start_id + iters_per_item * sg_size <= n);
    constexpr bool is_inclusive = false;

    if (is_full_thread) {
        T v = input[start_id];
        sg_scan(sg, v, op, init_present, carry, is_inclusive, slm);
        output[start_id] = v;

        for (std::uint32_t j = 1; j < iters_per_item; j++) {
            v = input[start_id + j * sg_size];
            sg_scan(sg, v, op, true, carry, is_inclusive, slm);
        ...
        if (sg_id < active_subgroups) {
            ...
            if (iters == 1) {
                std::size_t local_id = (start_id < n) ? start_id : n - 1;
                T v = input[local_id];
                sg_scan_partial(sg, v, op, init_present, carry,
                                         static_cast<std::uint32_t>(n - subgroup_start_id),
                                         is_inclusive, slm);
                if (start_id < n)
                    output[start_id] = v;
            } else {
                ...
                sg_scan_partial(sg, v, op, true, carry,
                    static_cast<std::uint32_t>(n - (subgroup_start_id + (iters - 1) * sg_size)),
                    is_inclusive, slm);
                if (offset < n)
                    output[offset] = v;
            }
...
int run_rts_reproducer() {
    ...
    std::printf("[bridge6a] n=%zu, num_work_groups=%u, work_group_size=%u\n", N, num_work_groups, work_group_size);
    std::printf("[bridge6a] min_sg=%u, max_sg=%u, max_num_sg_local=%u, max_num_sg_global=%u\n",
                (unsigned)min_sg_size, (unsigned)max_sg_size, max_num_sg_local, max_num_sg_global);
    std::printf("[bridge6a] max_inputs_per_item=%u, inputs_per_item=%u, block_size=%zu, num_blocks=%zu\n",
                (unsigned)max_inputs_per_item, inputs_per_item, block_size, num_blocks);
    std::printf("[bridge6a] scratch_size=%u\n", scratch_size);
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        std::printf("[bridge6a] Block %zu: workitems=%u global_range=%u inputs_remaining=%zu\n",
                    b, workitems_in_block, workitems_round_up, ir);

        // ============ REDUCE KERNEL ============
        // Reduce phase is identical to inclusive — it only accumulates carries.
        auto reduce_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ReduceK6a>(nd_range, [=](sycl::nd_item<1> ndi) {
        ...
        auto scan_ev = q.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<ScanK6a>(nd_range, [=](sycl::nd_item<1> ndi) {
                auto sg = ndi.get_sub_group();
                std::uint8_t sg_size = sg.get_max_local_range()[0];
                std::uint8_t sg_lid = sg.get_local_linear_id();
                std::uint32_t sg_id = sg.get_group_linear_id();
                ...
                // Key difference from inclusive: block 0, group 0, sg 0 gets
                // carry = init instead of being uninitialized.
                if (cap_b == 0) {
                    if (sg_id > 0) {
                        ...
                        // EXCLUSIVE: set carry to init, keep initialized
                        sub_group_carry = cap_init;
                        sg_carry_initialized = true;
                    }
                } else {
                ...
                // EXCLUSIVE: use exclusive scan variant
                scan_through_scan_exclusive(sg, std::plus<T>{}, sub_group_carry, sg_carry_initialized,
    ...
    for (std::size_t i = 0; i < N; i++) {
        if (h_output[i] != h_expected[i]) {
            if (errors < 20) {
                std::printf("[bridge6a] MISMATCH at [%zu]: got %d, expected %d\n", i, h_output[i], h_expected[i]);
            }
            errors++;
        }
    }
    ...
    if (errors == 0)
        std::printf("[bridge6a] PASS: all %zu elements match.\n", N);
    else
        std::printf("[bridge6a] FAIL: %d mismatches out of %zu elements.\n", errors, N);
