...
//
//===----------------------------------------------------------------------===//
// Isolation test: Full scan.pass logic but with reduce_then_scan debug output
// disabled. Rules out debug instrumentation (printf) as the cause.
...
struct test_inclusive_scan_plus
{
    template <typename Policy, typename Iterator1, typename Iterator2, typename Iterator3, typename Size, typename T>
    std::enable_if_t<!TestUtils::is_reverse_v<Iterator1>>
    operator()(Policy&& exec, Iterator1 in_first, Iterator1 in_last, Iterator2 out_first, Iterator2 out_last,
...
struct test_exclusive_scan_plus
{
    template <typename Policy, typename Iterator1, typename Iterator2, typename Iterator3, typename Size, typename T>
    std::enable_if_t<!TestUtils::is_reverse_v<Iterator1>>
    operator()(Policy&& exec, Iterator1 in_first, Iterator1 in_last, Iterator2 out_first, Iterator2 out_last,
...
main()
{
    // Matrix2x2 tests (same as scan.pass)
    {
        ...
        for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
        {
            ...
            invoke_on_all_policies<1>()(test_exclusive_scan_matrix<T>(), in.begin(), in.end(), out.begin(),
                                        out.end(), expected.begin(), expected.end(), in.size(), init, binary_op, trash);
#endif
        }
    }
    ...
    // int32 tests
    {
        ...
        for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
        {
            ...
            invoke_on_all_policies<2>()(test_inclusive_scan_plus(), in.begin(), in.end(), out.begin(),
                                        out.end(), expected.begin(), expected.end(), in.size(), init, trash);
            invoke_on_all_policies<3>()(test_exclusive_scan_plus(), in.begin(), in.end(), out.begin(),
                                        out.end(), expected.begin(), expected.end(), in.size(), init, trash);
        }
    }
