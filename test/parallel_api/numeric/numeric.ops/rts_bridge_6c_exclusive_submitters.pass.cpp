// -*- C++ -*-
//===-- rts_bridge_6c_exclusive_submitters.pass.cpp -------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 6c: Calls __parallel_transform_reduce_then_scan directly with
// buffer-backed all_view ranges for exclusive scan. Based on
// rts_bridge_4b_submitters.pass.cpp.
// Key changes from bridge 4b:
//   - __init_value<T> instead of __no_init_value<T>
//   - std::false_type{} for _Inclusive instead of std::true_type{}
...
// Kernel name tag
struct Bridge6cKernel;

int run_test() {
    using T = std::int32_t;
    using _UnaryOp = oneapi::dpl::identity;
    using _BinaryOp = std::plus<T>;
    using _InitType = oneapi::dpl::unseq_backend::__init_value<T>;  // EXCLUSIVE: init value
    ...
    _WriteOp write_op{};
    _InitType init{T{0}};  // EXCLUSIVE: init value of 0

    std::printf("[bridge6c] Calling __parallel_transform_reduce_then_scan directly (exclusive)\n");

    auto future = rts::__parallel_transform_reduce_then_scan<sizeof(T), Bridge6cKernel>(
        q, N, in_view, out_view,
        gen_input, binary_op, gen_input, scan_xform, write_op,
        init, /*_Inclusive=*/std::false_type{}, /*_IsUniquePattern=*/std::false_type{});
    ...
    for (std::size_t i = 0; i < N; i++) {
        if (h_out[i] != h_expected[i]) {
            if (errors < 20)
                std::printf("[bridge6c] MISMATCH [%zu]: got %d expected %d\n", i, h_out[i], h_expected[i]);
            errors++;
        }
    }
    std::printf("[bridge6c] %s: %d errors out of %zu\n", errors ? "FAIL" : "PASS", errors, N);
