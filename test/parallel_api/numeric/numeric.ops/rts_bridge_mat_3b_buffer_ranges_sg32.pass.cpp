// -*- C++ -*-
//===-- rts_bridge_mat_3b_buffer_ranges.pass.cpp ----------------------------===//
//
...
//
//===----------------------------------------------------------------------===//
// Bridge test 3b (Matrix2x2): Same as 3a (submitter structs with [=, *this])
// but uses SYCL buffer/accessor ranges instead of USM pointers.
// This isolates whether buffer accessors in the kernel capture cause the crash.
...
// ---------- Accessor-based gen/write ops ----------
template <typename T, typename Acc>
struct acc_gen_input
{
    using TempData = rts::__noop_temp_data;
    T
    operator()(const Acc& acc, std::size_t id, TempData&) const
    {
        return acc[id];
    }
};
...
struct acc_write_op
{
    ...
    void
    operator()(const Acc& acc, std::size_t id, const T& v, const _TempData&) const
    {
        acc[id] = v;
    }
};
...
// ---------- kernel names ----------
class ReduceK3bMatSg32;
class ScanK3bMatSg32;

// ---------- Submitter-style struct for the reduce kernel ----------
// Uses buffer accessors captured inside the lambda via [=, *this]
template <typename T, typename _BinaryOp>
struct reduce_submitter_3b
{
    ...
    template <typename InAcc, typename ScratchAcc>
    sycl::event
    submit(sycl::queue& __q, const sycl::nd_range<1> __nd_range,
           sycl::handler& cgh, InAcc __in_acc, ScratchAcc __scratch_acc,
           const std::size_t __inputs_remaining, const std::size_t __block_num) const
    {
        ...
        using GenInput = acc_gen_input<T, InAcc>;
        GenInput gen_input{};

        cgh.parallel_for<ReduceK3bMatSg32>(__nd_range, [=, *this](sycl::nd_item<1> ndi) [[sycl::reqd_sub_group_size(32)]] {
            ...
            if (sid < asg) {
                rts::__scan_through_elements_helper<__use_subgroup_ops, __is_inclusive,
                    ...
                    sg, gen_input, _ScanInputTransform{}, __binary_op, nullptr, carry,
                    __in_acc, /*unused*/ __in_acc, stid, cn, lip, sgs, sid, asg, sl);
        ...
        // Event returned by caller via q.submit
        return sycl::event{};
    }
};
...
struct scan_submitter_3b
{
    ...
    template <typename InAcc, typename OutAcc, typename ScratchAcc, typename ResAcc>
    void
    submit_body(sycl::handler& cgh, const sycl::nd_range<1> __nd_range,
                InAcc __in_acc, OutAcc __out_acc, ScratchAcc __scratch_acc, ResAcc __res_acc,
                const std::size_t __inputs_remaining, const std::size_t __block_num) const
    {
        ...
        using GenInput = acc_gen_input<T, InAcc>;
        using WriteOp = acc_write_op<T, OutAcc>;
        GenInput gen_input{};
        WriteOp write_op{};

        cgh.parallel_for<ScanK3bMatSg32>(__nd_range, [=, *this](sycl::nd_item<1> ndi) [[sycl::reqd_sub_group_size(32)]] {
            auto sg = ndi.get_sub_group();
            std::uint8_t ssz = sg.get_max_local_range()[0];
            std::uint8_t slid = sg.get_local_linear_id();
            std::uint32_t sid = sg.get_group_linear_id();
            ...
            if (sgci) {
                rts::__scan_through_elements_helper<__use_subgroup_ops, __is_inclusive,
                    ...
                    sg, gen_input, _ScanInputTransform{}, __binary_op, write_op, sgc,
                    __in_acc, __out_acc, stid, cn, lip, sgs, sid, asg, sl);
            } else {
                rts::__scan_through_elements_helper<__use_subgroup_ops, __is_inclusive,
...
int run_test() {
    ...
    std::printf("[bridge3b_mat_sg32] Device: %s\n", dev.get_info<sycl::info::device::name>().c_str());
    ...
    std::printf("[bridge3b_mat_sg32] wg_size=%u num_wg=%u ipi=%u blocks=%zu scratch=%u\n",
    ...
    reduce_submitter_3b<T, _BinaryOp> reduce_sub{
        num_wg, wg_size, max_per_block, max_sg_local, N, true, binary_op
    };
    scan_submitter_3b<T, _BinaryOp> scan_sub{
        num_wg, wg_size, max_per_block, max_sg_local, max_sg_global,
        num_blocks, N, true, binary_op
    };
    ...
    for (std::size_t b = 0; b < num_blocks; b++) {
        ...
        q.submit([&](sycl::handler& cgh) {
            auto in_acc = buf_in.template get_access<sycl::access::mode::read>(cgh);
            auto scratch_acc = buf_scratch.template get_access<sycl::access::mode::read_write>(cgh);
            reduce_sub.submit(q, ndr, cgh, in_acc, scratch_acc, ir, b);
        });

        // Scan kernel
        q.submit([&](sycl::handler& cgh) {
            auto in_acc = buf_in.template get_access<sycl::access::mode::read>(cgh);
            auto out_acc = buf_out.template get_access<sycl::access::mode::read_write>(cgh);
            auto scratch_acc = buf_scratch.template get_access<sycl::access::mode::read_write>(cgh);
            auto res_acc = buf_result.template get_access<sycl::access::mode::write>(cgh);
            scan_sub.submit_body(cgh, ndr, in_acc, out_acc, scratch_acc, res_acc, ir, b);
        });
