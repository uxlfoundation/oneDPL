...
template <typename Policy, sycl::usm::alloc alloc_type, typename KernelName>
void
test_with_usm(Policy&& policy)
{
    ...
    TestUtils::usm_data_transfer<alloc_type, int> dt_helper5(policy.queue(), output_values1, n);
    TestUtils::usm_data_transfer<alloc_type, int> dt_helper6(policy.queue(), output_values2, n);
    ...
    oneapi::dpl::exclusive_scan_by_segment(
        CLONE_NEW_POLICY_WITH_NAME(policy, KernelName),
        begin_keys_in, end_keys_in,
        begin_vals_in, begin_vals_out, std::make_tuple(int(1), int(1)),
    ...
    // {21, 21}: {1, 6}
    // {37, 37}: {1, 8}
    const int exp_values1[n] = {1, 1, 1, 1, 1, 1, 6, 1, 8};
    const int exp_values2[n] = {1, 1, 1, 1, 1, 1, 6, 1, 8};
    EXPECT_EQ_N(exp_values1, output_values1, n, "wrong values1 from exclusive_scan_by_segment");
    EXPECT_EQ_N(exp_values2, output_values2, n, "wrong values2 from exclusive_scan_by_segment");
}
