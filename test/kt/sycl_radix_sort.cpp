...
test_sycl_iterators(sycl::queue q, std::size_t size, KernelParam param)
{
    ...
    {
        sycl::buffer<T> buf(input.data(), input.size());
        oneapi::dpl::experimental::kt::gpu::radix_sort<IsAscending, RadixBits>(q, oneapi::dpl::begin(buf),
                                                                               oneapi::dpl::end(buf), param)
            .wait();
    }
...
test_sycl_buffer(sycl::queue q, std::size_t size, KernelParam param)
{
    ...
    {
        sycl::buffer<T> buf(input.data(), input.size());
        oneapi::dpl::experimental::kt::gpu::radix_sort<IsAscending>(q, buf, param).wait();
    }
...
test_small_sizes(sycl::queue q, KernelParam param)
{
    ...
    oneapi::dpl::experimental::kt::gpu::radix_sort<IsAscending, RadixBits>(q, oneapi::dpl::begin(input),
