...
// ATTENTION: this container can be used only with hetero execution policies
template <typename _PolicySource, typename _PolicyNew>
struct policy_container
{
    using _PolicyNewDecayed = std::decay_t<_PolicyNew>;

    static_assert(oneapi::dpl::__internal::__is_hetero_execution_policy_v<std::decay_t<_PolicySource>),
        "policy_container can be used only with hetero execution policies.");
...
// Clone hetero policy and pass cloned instance as l-value / r-value
...
// Simple pass non-hetero policy as l-value / r-value
#define CLONE_NEW_POLICY(policy_src) policy_src
