    ...
    bool ret = true;
    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<class KernelTest>([=]() {
                char s[] = "1234567890";
                ret_access[0] &= test(input_iterator<char*>(s), input_iterator<char*>(s), false);
                ret_access[0] &= test(input_iterator<char*>(s), input_iterator<char*>(s + 1), true);
                ret_access[0] &= test(forward_iterator<char*>(s), forward_iterator<char*>(s), false);
                ret_access[0] &= test(forward_iterator<char*>(s), forward_iterator<char*>(s + 1), true);
                ret_access[0] &= test(bidirectional_iterator<char*>(s), bidirectional_iterator<char*>(s), false);
                ret_access[0] &= test(bidirectional_iterator<char*>(s), bidirectional_iterator<char*>(s + 1), true);
